 ...
 */
$sub_part = '';
require __DIR__ . '/db_details_links.php';
...
if (isset($submit_search)) {
     ...
     *
     * @param mixed $table
     * @param mixed $search_str
     * @param mixed $search_option
     *
    ...
    function PMA_getSearchSqls($table, $search_str, $search_option)
    {
        ...
        $like_or_regex = ((4 == $search_option) ? 'REGEXP' : 'LIKE');
        ...
        $implode_str = (1 == $search_option ? ' OR ' : ' AND ');
...
<br>
        ...
        if (isset($onetable)) {
            ...
            echo '    ' . sprintf($strSearchResultsFor, htmlspecialchars($original_search_str, ENT_QUOTES | ENT_HTML5), $option_str) . "\n";
            ...
            echo '    <!-- Search results in table ' . $onetable . ' (' . $res_cnt . ') -->' . "\n"
                 . '    <br>' . "\n"
                 . '    <table><tr><td>' . sprintf($strNumSearchResultsInTable, $res_cnt, htmlspecialchars($onetable, ENT_QUOTES | ENT_HTML5)) . "</td>\n";

            if ($res_cnt > 0) {
                echo '<td>' . PMA_linkOrButton(
                    'sql.php?' . $url_sql_query
            ...
            if (isset($table_select)) {
                ...
                for ($i = 0; $i < $num_selectedtables; $i++) {
                    ...
                    echo '        <!-- Search results in table ' . $table_select[$i] . ' (' . $res_cnt . ') -->' . "\n"
                     . '        <li>' . "\n"
                     . '            <table><tr><td>' . sprintf($strNumSearchResultsInTable, $res_cnt, htmlspecialchars($table_select[$i], ENT_QUOTES | ENT_HTML5)) . "</td>\n";
 ...
 */
echo "\n";
$searched = (isset($original_search_str)) ? htmlspecialchars($original_search_str, ENT_QUOTES | ENT_HTML5) : '';
...
<form method="post" action="db_search.php" name="db_search">
    ...
    <tr><td colspan="2"></td></tr><tr>
        ...
        <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
            <input type="radio" id="search_option_1" name="search_option" value="1"<?php if (1 == $search_option) {
    echo ' checked';
} ?>>
            <label for="search_option_1"><?php echo $strSearchOption1; ?></label>&nbsp;*<br>
            <input type="radio" id="search_option_2" name="search_option" value="2"<?php if (2 == $search_option) {
    echo ' checked';
} ?>>
            <label for="search_option_2"><?php echo $strSearchOption2; ?></label>&nbsp;*<br>
            <input type="radio" id="search_option_3" name="search_option" value="3"<?php if (3 == $search_option) {
    echo ' checked';
} ?>>
            <label for="search_option_3"><?php echo $strSearchOption3; ?></label><br>
            <input type="radio" id="search_option_4" name="search_option" value="4"<?php if (4 == $search_option) {
    echo ' checked';
} ?>>
...
if ($num_tables > 1) {
    ...
    while ($i < $num_tables) {
        if (!empty($unselectall)) {
            ...
            if ((isset($table_select) && -1 != PMA_isInto($tables[$i], $table_select))
                || (!empty($selectall))
        ...
        echo '                <option value="' . htmlspecialchars($tables[$i], ENT_QUOTES | ENT_HTML5) . '"' . $is_selected . '>' . htmlspecialchars($tables[$i], ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
    ...
    echo '            ' . htmlspecialchars($tables[0], ENT_QUOTES | ENT_HTML5) . "\n";

    echo '            <input type="hidden" name="table" value="' . htmlspecialchars($tables[0], ENT_QUOTES | ENT_HTML5) . '">' . "\n";
} // end if... else...
