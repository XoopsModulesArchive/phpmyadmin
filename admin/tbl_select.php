 ...
 */
$numfunctions = ['=', '>', '>=', '<', '<=', '!=', 'LIKE', 'NOT LIKE'];
$textfunctions = ['LIKE %...%', 'LIKE', 'NOT LIKE', '=', '!=', 'REGEXP', 'NOT REGEXP'];
$enumfunctions = ['=', '!='];
$nullfunctions = ['IS NULL', 'IS NOT NULL'];
$unaryfunctions = [
    'IS NULL' => 1,
'IS NOT NULL' => 1,
];
...
if (!isset($param) || '' == $param[0]) {
    ...
    while (false !== ($row = PMA_DBI_fetch_assoc($result))) {
        ...
        if (0 == strncasecmp($type, 'set', 3)
            ...
            $type = str_replace(['binary', 'zerofill', 'unsigned'], '', mb_strtolower($type));
        }
        ...
        if (PMA_MYSQL_INT_VERSION >= 40100 && !empty($row['Collation']) && 'NULL' != $row['Collation']) {
...
<form method="post" action="tbl_select.php" name="insertForm">
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        echo '                            '
            . '<option value="' . htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5) . '" selected="selected">' . htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
    } ?>
                            </select>
                        </td>
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        ...
        echo '<option value="' . htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
    } // end for?>
                            </select><br>
                            <input type="radio" name="order" value="ASC" checked id="sortASC">
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        ...
        $bgcolor = ($i % 2) ? $cfg['BgcolorOne'] : $cfg['BgcolorTwo']; ?>
            <tr>
                <td bgcolor="<?php echo $bgcolor; ?>"><b><?php echo htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5); ?></b></td>
                ...
                <?php echo PMA_MYSQL_INT_VERSION >= 40100 ? '<td bgcolor="' . $bgcolor . '">' . $fields_collation[$i] . '</td>' . "\n" : ''; ?>
                 <td bgcolor="<?php echo $bgcolor; ?>">
                    <select name="func[]">
        <?php
        if (0 == strncasecmp($fields_type[$i], 'enum', 4)) {
            foreach ($enumfunctions as $k => $fc) {
                echo "\n" . '                        '
                   . '<option value="' . htmlspecialchars($fc, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($fc, ENT_QUOTES | ENT_HTML5) . '</option>';
            }
        ...
        <?php
                ...
                if (0 == strncasecmp($fields_type[$i], 'enum', 4)) {
                    ...
                    $enum_value = explode(', ', str_replace("'", '', mb_substr($fields_type[$i], 5, -1)));
                    ...
                    <input type="hidden" name="names[<?php echo $i; ?>]" value="<?php echo htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5); ?>">
 ...
 */
    ...
    if ('' != trim($where)) {
        ...
        while (list($i, $func_type) = each($func)) {
            if (1 == @$unaryfunctions[$func_type]) {
                $fields[$i] = '';

                $w[] = PMA_backquote(urldecode($names[$i])) . ' ' . $func_type;
            } elseif (0 == strncasecmp($types[$i], 'enum', 4)) {
                if (!empty($fields[$i])) {
