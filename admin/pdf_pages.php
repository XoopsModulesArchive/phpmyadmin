...
/* $Id: pdf_pages.php,v 2.10 2004/05/20 16:14:08 nijel Exp $ */
...
if ($cfgRelation['pdfwork']) {
        ...
        switch ($do) {
            ...
            case 'createpage':
                ...
                if (isset($autolayout)) {
                    ...
                    $pdf_page_number = PMA_DBI_insert_id(($dbh ?? ''));
                    ...
                    if ($master_tables_rs && PMA_DBI_num_rows($master_tables_rs) > 0) {
                        ...
                        foreach ($all_tables as $master_table) {
                            ...
                            foreach ($foreigners as $foreigner) {
                                if (!in_array($foreigner['foreign_table'], $foreign_tables, true)) {
                        ...
                        foreach ($foreign_tables as $foreign_table) {
                            if (!in_array($foreign_table, $all_tables, true)) {
                ...
                for ($i = 0; $i < $c_table_rows; $i++) {
                    ...
                    if (isset($arrvalue['name']) && '--' != $arrvalue['name']) {
                        ...
                        if ($test_rs && PMA_DBI_num_rows($test_rs) > 0) {
                            if (isset($arrvalue['delete']) && 'y' == $arrvalue['delete']) {
                                $ch_query = 'DELETE FROM ' . PMA_backquote($cfgRelation['table_coords'])
                                          . ' WHERE db_name = \'' . PMA_sqlAddslashes($db) . '\''
                                          . ' AND   table_name = \'' . PMA_sqlAddslashes($arrvalue['name']) . '\''
                                          . ' AND   pdf_page_number = ' . $chpage;
    ...
    while (false !== ($val = @PMA_DBI_fetch_row($alltab_rs))) {
        $selectboxall[] = $val[0];
    }
...
<form method="get" action="pdf_pages.php" name="selpage">
    ...
    <select name="chpage" onchange="this.form.submit()">
        <?php
        while (false !== ($curr_page = PMA_DBI_fetch_assoc($page_rs))) {
            echo "\n" . '        '
    ...
    <input type="radio" name="action_choose" value="0" id="radio_choose0" checked style="vertical-align: middle"> <label for="radio_choose0">
...
$page_query = 'SELECT * FROM ' . PMA_backquote($cfgRelation['table_coords'])
        ...
        while (false !== ($temp_sh_page = @PMA_DBI_fetch_assoc($page_rs))) {
...
foreach ($array_sh_page as $key => $temp_sh_page) {
                ...
                while (false !== ($row = PMA_DBI_fetch_assoc($fields_rs))) {
                    echo '<br>' . htmlspecialchars($row['Field'], ENT_QUOTES | ENT_HTML5) . "\n";
                }
...
<form method="post" action="pdf_pages.php" name="edcoord">
    ...
    <tr>
        ...
        echo($cfg['WYSIWYG-PDF'] ? "\n" . '    <input type="hidden" name="showwysiwyg" value="' . ((isset($showwysiwyg) && '1' == $showwysiwyg) ? '1' : '0') . '">' : '');
    ...
    if (isset($do)
    && ('edcoord' == $do
       || ('choosepage' == $do && isset($chpage))
       || ('createpage' == $do && isset($chpage)))) {
        ?>
<form method="post" action="pdf_schema.php" name="pdfoptions">
    ...
    <input type="checkbox" name="show_color" id="show_color_opt" checked>
    ...
    <input type="checkbox" name="with_doc" id="with_doc" checked>
...
<?php
    ...
    &nbsp;&nbsp;<input type="submit" value="<?php echo $strGo; ?>">
        ...
        <?php
        if ((isset($showwysiwyg) && '1' == $showwysiwyg)) {
            ?>
<script type="text/javascript">
<!--
