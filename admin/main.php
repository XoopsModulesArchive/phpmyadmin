...
if ($server > 0) {
    ...
    [$mysql_cur_user_and_host] = PMA_DBI_fetch_row($res);

    $mysql_cur_user = mb_substr($mysql_cur_user_and_host, 0, mb_strrpos($mysql_cur_user_and_host, '@'));
 ...
 */
if (($server > 0) && isset($mode) && ('reload' == $mode)) {
...
require __DIR__ . '/libraries/select_server.lib.php';
...
if ($server > 0) {
    ...
    if ($dbh) {
        ...
        if ($rs_usr) {
            while (false !== ($result_usr = PMA_DBI_fetch_assoc($rs_usr))) {
                if (!$is_create_priv) {
                    $is_create_priv = ('Y' == $result_usr['Create_priv']);
                }

                if (!$is_reload_priv) {
                    $is_reload_priv = ('Y' == $result_usr['Reload_priv']);
                }
            } // end while
    ...
    if (!$is_create_priv) {
        ...
        if ($rs_usr) {
            ...
            while (false !== ($row = PMA_DBI_fetch_assoc($rs_usr))) {
                if (preg_match($re0 . '(%|_)', $row['Db'])
                    || (!PMA_DBI_try_query('USE ' . preg_replace($re1 . '(%|_)', '\\1\\3', $row['Db'])) && 1044 != mb_substr(PMA_DBI_getError(), 1, 4))) {
            ...
            if ($rs_usr) {
                $re0 = '(^|(\\\\\\\\)+|[^\])'; // non-escaped wildcards
                $re1 = '(^|[^\])(\\\)+'; // escaped wildcards
                while (false !== ($row = PMA_DBI_fetch_row($rs_usr))) {
                    $show_grants_dbname = mb_substr($row[0], mb_strpos($row[0], ' ON ') + 4, (mb_strpos($row[0], '.', mb_strpos($row[0], ' ON ')) - mb_strpos($row[0], ' ON ') - 4));

                    $show_grants_dbname = preg_replace('^`(.*)`', '\\1', $show_grants_dbname);

                    $show_grants_str = mb_substr($row[0], 6, (mb_strpos($row[0], ' ON ') - 6));

                    if (('ALL' == $show_grants_str) || ('ALL PRIVILEGES' == $show_grants_str) || ('CREATE' == $show_grants_str) || mb_strpos($show_grants_str, 'CREATE')) {
                        ...
                        if ((preg_match($re0 . '%|_', $show_grants_dbname)
                                  && !preg_match('\\\\%|\\\\_', $show_grants_dbname)) || (!PMA_DBI_try_query('USE ' . preg_replace($re1 . '(%|_)', '\\1\\3', $show_grants_dbname)) && 1044 != mb_substr(PMA_DBI_getError(), 1, 4))
                            ) {
    ...
    if ('config' == $cfg['Server']['auth_type']) {
...
<?php
        ...
        if ('config' != $cfg['Server']['auth_type']) {
            $http_logout = ('http' == $cfg['Server']['auth_type']) ? "\n"
...
if (isset($cfg['AllowAnywhereRecoding']) && $cfg['AllowAnywhereRecoding']
    ...
    <?php
       ...
       . '            ' . $strMySQLCharset . ': '
       . '            <b>'
       . '               ' . $mysql_charsets_descriptions[$mysql_charset_map[mb_strtolower($charset)]] . "\n"
       . '               (' . $mysql_charset_map[mb_strtolower($charset)] . ')' . "\n"
       . '            </b>' . "\n"
       . '        </td>' . "\n"
       . '    </tr>' . "\n"
...
if (isset($available_themes_choices) && $available_themes_choices > 1) {
    ...
    <tr>
<?php
        echo '        ' . ('' != $str_iconic_list ? sprintf($str_iconic_list, $theme_preview_href, 's_theme.png', ($strTheme ?? 'Theme (Style)'), '</a>') : $str_normal_list) . "\n"; ?>
        <td>
