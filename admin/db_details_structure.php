...
/* $Id: db_details_structure.php,v 2.33 2004/06/24 14:17:39 lem9 Exp $ */
 ...
 * Rename database
 */
if (isset($db) && isset($db_rename) && 'true' == $db_rename) {
    if (!isset($newname) || empty($newname)) {
        ...
        $message = sprintf($strRenameDatabaseOK, htmlspecialchars($db, ENT_QUOTES | ENT_HTML5), htmlspecialchars($newname, ENT_QUOTES | ENT_HTML5));
 ...
 */
if ($cfgRelation['commwork'] && isset($db_comment) && 'true' == $db_comment) {
...
<?php
    ...
    foreach ($tables as $keyname => $sts_data) {
        ...
        $table_name = htmlspecialchars($table, ENT_QUOTES | ENT_HTML5);

        $alias = (!empty($tooltip_aliasname) && isset($tooltip_aliasname[$table])) ? htmlspecialchars($tooltip_aliasname[$table], ENT_QUOTES | ENT_HTML5) : htmlspecialchars($sts_data['Name'], ENT_QUOTES | ENT_HTML5);

        $truename = (!empty($tooltip_truename) && isset($tooltip_truename[$table])) ? htmlspecialchars($tooltip_truename[$table], ENT_QUOTES | ENT_HTML5) : htmlspecialchars($sts_data['Name'], ENT_QUOTES | ENT_HTML5);
        ...
        if ($num_columns > 0 && $num_tables > $num_columns && (0 == ($row_count % ($num_columns)))) {
    ...
    <td valign="top">
        ...
        if (!empty($sts_data['Rows'])) {
            echo '<a href="sql.php?' . $tbl_url_query . '&amp;sql_query='
                 . (isset($book_sql_query) && false !== $book_sql_query ? urlencode($book_sql_query) : urlencode('SELECT * FROM ' . PMA_backquote($table)))
                ...
                <td align="center" bgcolor="<?php echo $bgcolor; ?>">
                    ...
                    <a href="tbl_properties_structure.php?<?php echo $tbl_url_query; ?>">
                        <?php echo $titles['Properties']; ?></a>
                            ...
                            <td align="center" bgcolor="<?php echo $bgcolor; ?>">
                    <a href="sql.php?<?php echo $tbl_url_query; ?>&amp;reload=1&amp;purge=1&amp;sql_query=<?php echo urlencode('DROP TABLE ' . PMA_backquote($table)); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strTableHasBeenDropped, htmlspecialchars($table, ENT_QUOTES | ENT_HTML5))); ?>"
        ...
        if (!empty($sts_data['Rows'])) {
            ...
            if (PMA_MYSQL_INT_VERSION >= 40000) {
                echo urlencode('TRUNCATE ' . PMA_backquote($table))
                     . '&amp;zero_rows='
                     . urlencode(sprintf($strTableHasBeenEmptied, htmlspecialchars($table, ENT_QUOTES | ENT_HTML5)))
        ...
        if (isset($sts_data['Rows'])) {
            ...
            if (isset($sts_data['Type']) && preg_match('@^(MyISAM|ISAM|HEAP)$@', $sts_data['Type'])) {
                if ($cfg['ShowStats']) {
                    $tblsize = (float)$sts_data['Data_length'] + (float)$sts_data['Index_length'];

                    $sum_size += $tblsize;

                    [$formated_size, $unit] = PMA_formatByteDown($tblsize, 3, ($tblsize > 0) ? 1 : 0);

                    if (isset($sts_data['Data_free']) && $sts_data['Data_free'] > 0) {
                        [$formated_overhead, $overhead_unit] = PMA_formatByteDown($sts_data['Data_free']);
            ...
            // sizes are.
                ...
                if (isset($sts_data['Type']) && 'InnoDB' == $sts_data['Type']) {
                    if ($cfg['ShowStats']) {
                    ...
                    if ($sts_data['Rows'] < $cfg['MaxExactCount']) {
                        ...
                        [$row_count] = PMA_DBI_fetch_row($table_info_result);
    ...
    if ($cfg['ShowStats']) {
        ...
        [$overhead_formated, $overhead_unit] = PMA_formatByteDown($overhead_size, 3, 1);
    }
...
<table border="0" cellpadding="2" cellspacing="0">
    <!-- Create a new table -->
     ...
     <td class="tblHeaders" colspan="3" nowrap="nowrap"><?php
        ...
        if ($cfg['PropertiesIconic']) {
            echo '<img src="' . $pmaThemeImage . 'b_newtbl.png" border="0" width="16" height="16" hspace="2" align="absmiddle">';
        }
        // if you want navigation:
        $strDBLink = '<a href="' . $GLOBALS['cfg']['DefaultTabDatabase'] . '?' . PMA_generate_common_url() . '&amp;db=' . urlencode($GLOBALS['db']) . '">'
                     . htmlspecialchars($GLOBALS['db'], ENT_QUOTES | ENT_HTML5) . '</a>';
        // else use
...
if ($cfgRelation['commwork']) {
    ...
    echo $strDBComment; ?></td></tr>
        ...
        <tr bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
            ...
            <?php echo PMA_generate_common_hidden_inputs($db); ?>
            <input type="text" name="comment" class="textfield" size="30" value="<?php echo(isset($comment) && is_array($comment) ? htmlspecialchars(implode(' ', $comment), ENT_QUOTES | ENT_HTML5) : ''); ?>"></td><td align="right">
            <input type="submit" value="<?php echo $strGo; ?>">
         </td></tr>
        </form>
<?php
...
if ($num_tables > 0
    && !$cfgRelation['allworks'] && false === $cfg['PmaNoRelation_DisableWarning']) {
...
(
    $num_tables > 0 && $cfgRelation['relwork'] && $cfgRelation['commwork'] && isset($cfg['docSQLDir']) && !empty($cfg['docSQLDir'])
)
...
<?php
        ...
        echo "\n"; ?>
            ...
            <tr bgcolor="<?php echo $cfg['BgcolorTwo']; ?>">
                                                  <td width="20">&nbsp;</td><td width="20" valign="top">
            <input type="checkbox" name="show_color" id="show_color_opt" checked></td><td>
            ...
            <tr bgcolor="<?php echo $cfg['BgcolorTwo']; ?>">
                                                  <td width="20">&nbsp;</td><td width="20" valign="top">
            <input type="checkbox" name="with_doc" id="with_doc" checked></td><td>
...
if ($num_tables > 0
    && $cfgRelation['relwork'] && $cfgRelation['commwork'] && isset($cfg['docSQLDir']) && !empty($cfg['docSQLDir'])
    ) {
    ?>
    <!-- import docSQL files -->
