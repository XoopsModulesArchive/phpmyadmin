...
if (isset($submit)) {
    ...
    for ($i = 0; $i < $field_cnt; $i++) {
        ...
        if (0 == strcmp(str_replace('"', '&quot;', $field_orig[$i]), $field_name[$i])) {
            $field_name[$i] = $field_orig[$i];
        }

        $field_default_orig[$i] = urldecode($field_default_orig[$i]);

        if (0 == strcmp(str_replace('"', '&quot;', $field_default_orig[$i]), $field_default[$i])) {
            $field_default[$i] = $field_default_orig[$i];
        }

        $field_length_orig[$i] = urldecode($field_length_orig[$i]);

        if (0 == strcmp(str_replace('"', '&quot;', $field_length_orig[$i]), $field_length[$i])) {
        ...
        if ('' != $field_attribute[$i]) {
            ...
            if (PMA_MYSQL_INT_VERSION >= 40100 && '' != $field_collation[$i]) {
                $query .= PMA_generateCharsetQueryPart($field_collation[$i]);
            }
        }

        if ('' != $field_default[$i]) {
            if ('NULL' == mb_strtoupper($field_default[$i])) {
                $query .= ' DEFAULT NULL';
            } else {
...
if (false === $abort) {
    ...
    require __DIR__ . '/tbl_properties.inc.php';
}
