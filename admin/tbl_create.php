...
if (isset($submit_num_fields)) {
    ...
    for ($i = 0; $i < $field_cnt; $i++) {
        ...
        if ('' != $field_attribute[$i]) {
            ...
            if (PMA_MYSQL_INT_VERSION >= 40100 && !empty($field_collation[$i])) {
        ...
        if (!empty($tbl_type) && ('Default' != $tbl_type)) {
        ...
        if (false === $error_create) {
            ...
            $message = $strTable . ' ' . htmlspecialchars($table, ENT_QUOTES | ENT_HTML5) . ' ' . $strHasBeenCreated;
            ...
            require __DIR__ . '/' . $cfg['DefaultTabTable'];
