...
if (0 == $num_tables && empty($db_query_force)) {
    $is_info = true;

    require __DIR__ . '/db_details_structure.php';
...
$strHiddenFields = '        <input type="hidden" name="is_js_confirmed" value="0">' . "\n"
                 ...
                 . '        <input type="hidden" name="zero_rows" value="' . htmlspecialchars($strSuccess, ENT_QUOTES | ENT_HTML5) . '">' . "\n"
                 . '        <input type="hidden" name="prev_sql_query" value="' . ((!empty($query_to_display)) ? htmlspecialchars($query_to_display, ENT_QUOTES | ENT_HTML5) : '') . '">' . "\n";
?>
...
<tr><td class="tblHeaders" colspan="2">
    <?php
           ...
           $strDBLink = '<a href="' . $GLOBALS['cfg']['DefaultTabDatabase'] . $header_url_qry . '&amp;db=' . urlencode($db) . '">'
                        . htmlspecialchars($db, ENT_QUOTES | ENT_HTML5) . '</a>';
           // else use
  ...
  <td colspan="2" align="center">
    ...
    if ($is_bzip || $is_gzip) {
        ...
        echo '        &nbsp;&nbsp;' . $strCompression . ':<br>&nbsp;&nbsp;&nbsp;' . "\n"
           . '            <input type="radio" id="radio_sql_file_compression_auto" name="sql_file_compression" value="" checked><label for="radio_sql_file_compression_auto">' . $strAutodetect . '</label>&nbsp;&nbsp;' . "\n"
...
if ($cfg['Bookmark']['db'] && $cfg['Bookmark']['table']) {
    if (($bookmark_list = PMA_listBookmarks($db, $cfg['Bookmark'])) && count($bookmark_list) > 0) {
        ...
        foreach ($bookmark_list as $key => $value) {
            echo '            <option value="' . htmlspecialchars($value, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($key, ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
        }
