...
if (isset($cfg['docSQLDir']) && !empty($cfg['docSQLDir'])) {
    ...
    function docsql_check($docpath = '', $file = '', $filename = '', $content = 'none')
    {
        ...
        if (preg_match('@^(.*)_field_comment\.(txt|zip|bz2|bzip).*$@i', $filename)) {
            ...
            if (isset($lines) && is_array($lines) && count($lines) > 0) {
                foreach ($lines as $lkey => $line) {
                    ...
                    if (!empty($inf[1]) && mb_strlen(trim($inf[1])) > 0) {
                        ...
                        if (PMA_query_as_cu($qry)) {
                            echo '<p>' . $GLOBALS['strAddedColumnComment'] . ' ' . htmlspecialchars($tab, ENT_QUOTES | ENT_HTML5) . '.' . htmlspecialchars($inf[0], ENT_QUOTES | ENT_HTML5) . '</p>';
                        } else {
                    ...
                    if (!empty($inf[2]) && mb_strlen(trim($inf[2])) > 0) {
                        $for = explode('->', $inf[2]);
                        ...
                        if (PMA_query_as_cu($qry)) {
                            echo '<p>' . $GLOBALS['strAddedColumnRelation'] . ' ' . htmlspecialchars($tab, ENT_QUOTES | ENT_HTML5) . '.' . htmlspecialchars($inf[0], ENT_QUOTES | ENT_HTML5) . ' to ' . htmlspecialchars($inf[2], ENT_QUOTES | ENT_HTML5) . '</p>';
                        } else {
        ...
        if ('none' != $content) {
            echo '<p><font color="orange">' . sprintf($GLOBALS['strIgnoringFile'], ' ' . htmlspecialchars($file, ENT_QUOTES | ENT_HTML5)) . '</font></p>' . "\n";
        } else {
    ...
    if (isset($do) && 'import' == $do) {
        ...
        if ('none' != $sql_file) {
            if (file_exists($sql_file)
                ...
                if (!isset($docsql_text) || false === $docsql_text || '' == $docsql_text) {
                    echo '<p><font color="red">' . $GLOBALS['strFileCouldNotBeRead'] . '</font></p>' . "\n";
                } else {
    ...
    <form method="post" action="db_details_importdocsql.php" <?php if ($is_upload) {
        ...
        <br><br>
        <?php echo $cfg['docSQLDir']; ?>/<input class="textfield" type="text" name="docpath" size="15" value="<?php echo($orig_docpath ?? ''); ?>">
    <?php
