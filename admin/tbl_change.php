 ...
 */
require __DIR__ . '/libraries/db_table_exists.lib.php';
...
$chg_evtHandler = (PMA_USR_BROWSER_AGENT == 'IE' && PMA_USR_BROWSER_VER >= 5) ? 'onpropertychange' : 'onchange';
...
<form method="post" action="tbl_replace.php" name="insertForm" <?php if ($is_upload) {
    ...
    <input type="hidden" name="pos" value="<?php echo $pos ?? 0; ?>">
    ...
    <input type="hidden" name="dontlimitchars" value="<?php echo($dontlimitchars ?? 0); ?>">
...
while (false !== ($trow = PMA_DBI_fetch_assoc($table_def))) {
    $trow_table_def[] = $trow;
}
...
foreach ($loop_array as $vrowcount => $vrow) {
    ...
    if ($insert_mode && $vrowcount > 0) {
        echo '<input type="checkbox" checked name="insert_ignore_' . $vrowcount . '" id="insert_ignore_check_' . $vrowcount . '">';
...
<?php
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        ...
        if ((0 == ($o_rows * $fields_cnt + $i) % 15) && (0 != $i + $o_rows)) {
            ?>
        <tr>
        ...
        if ('datetime' == $row_table_def['Type']
            && !isset($row_table_def['Default']) && isset($row_table_def['Null'])
            && 'YES' == $row_table_def['Null']) {
            $row_table_def['Default'] = null;
        }

        if ('datetime' == $row_table_def['Type']
            && (!isset($row_table_def['Default']))
            && (null !== $row_table_def['Default'])) {
            // INSERT case
            ...
            // UPDATE case with an empty and not NULL value under PHP4
                ...
                if (empty($vrow[$rowfield]) && null === $vrow[$rowfield]) {
                    $vrow[$rowfield] = date('Y-m-d H:i:s', time());
                }
            } // end if... else if...
        }
        ...
        <tr>
            <td <?php echo($cfg['LongtextDoubleTextarea'] && mb_strstr($row_table_def['True_Type'], 'longtext') ? 'rowspan="2"' : ''); ?> align="center" bgcolor="<?php echo $bgcolor; ?>"><?php echo htmlspecialchars($field, ENT_QUOTES | ENT_HTML5); ?></td>
        <?php
        echo "\n";

        // The type column

        $is_binary = mb_stristr($row_table_def['Type'], ' binary');

        $is_blob = mb_stristr($row_table_def['Type'], 'blob');

        $is_char = mb_stristr($row_table_def['Type'], 'char');
        ...
        if (isset($vrow)) {
            if (!isset($vrow[$rowfield])
              || (function_exists('is_null') && null === $vrow[$rowfield])) {
                ...
                $special_chars = htmlspecialchars($vrow[$rowfield], ENT_QUOTES | ENT_HTML5);
            ...
            $backup_field = ('timestamp' == $row_table_def['True_Type']) ? '' : '<input type="hidden" name="fields_prev' . $vkey . '[' . urlencode($field) . ']" value="' . urlencode($vrow[$rowfield]) . '">';
        } else {
            ...
            $special_chars = htmlspecialchars($row_table_def['Default'], ENT_QUOTES | ENT_HTML5);
        ...
        if ($cfg['ShowFunctionFields']) {
            if (($cfg['ProtectBinary'] && $is_blob)
                || ('all' == $cfg['ProtectBinary'] && $is_binary)) {
                echo '        <td align="center" bgcolor="' . $bgcolor . '">' . $strBinary . '</td>' . "\n";
            } else {
                if (mb_strstr($row_table_def['True_Type'], 'enum') || mb_strstr($row_table_def['True_Type'], 'set')) {
                    echo '        <td align="center" bgcolor="' . $bgcolor . '">--</td>' . "\n";
                } else {
                    ?>
            <td bgcolor="<?php echo $bgcolor; ?>">
                <select name="funcs<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab1); ?>" id="field_<?php echo $idindex; ?>_1">
                    <option></option>
                <?php
                echo "\n";
                    ...
                    if (isset($cfg['RestrictFunctions']) && isset($cfg['RestrictColumnTypes']) && isset($cfg['RestrictColumnTypes'][mb_strtoupper($row_table_def['True_Type'])]) && isset($cfg['RestrictFunctions'][$cfg['RestrictColumnTypes'][mb_strtoupper($row_table_def['True_Type'])]])) {
                        $current_func_type = $cfg['RestrictColumnTypes'][mb_strtoupper($row_table_def['True_Type'])];
                    ...
                    for ($j = 0; $j < $cnt_dropdown; $j++) {
                        ...
                        $selected = ($first_timestamp && $dropdown[$j] == $cfg['DefaultFunctions']['first_timestamp']) || (!$first_timestamp && $dropdown[$j] == $default_function) ? ' selected="selected"' : '';
                    ...
                    for ($j = 0; $j < $cnt_functions; $j++) {
                        if (!isset($dropdown_built[$cfg['Functions'][$j]]) || 'TRUE' != $dropdown_built[$cfg['Functions'][$j]]) {
                            // Is current function defined as default?

                            $selected = ($first_timestamp && $cfg['Functions'][$j] == $cfg['DefaultFunctions']['first_timestamp']) || (!$first_timestamp && $cfg['Functions'][$j] == $default_function) ? ' selected="selected"' : '';

                            if (true === $op_spacing_needed) {
                                echo '                ';
        ...
        if (!(($cfg['ProtectBinary'] && $is_blob) || ('all' == $cfg['ProtectBinary'] && $is_binary))
            && 'YES' == $row_table_def['Null']) {
            ...
            if (mb_strstr($row_table_def['True_Type'], 'enum')) {
                if (mb_strlen($row_table_def['Type']) > 20) {
                    $onclick .= '1, ';
                } else {
                    $onclick .= '2, ';
                }
            } else {
                if (mb_strstr($row_table_def['True_Type'], 'set')) {
                    $onclick .= '3, ';
                } else {
        ...
        if (isset($foreign_link) && true === $foreign_link) {
            ?>
            <td bgcolor="<?php echo $bgcolor; ?>">
            ...
            <input type="text"   name="field_<?php echo md5($field); ?><?php echo $vkey; ?>[]" class="textfield" <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab3); ?>" id="field_<?php echo($idindex); ?>_3" value="<?php echo htmlspecialchars(
                    $data,
            ...
            <input type="hidden" name="fields_type<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" value="foreign">
            <input type="hidden" name="fields<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" value="" id="field_<?php echo $idindex; ?>_1">
            <select name="field_<?php echo md5($field); ?><?php echo $vkey; ?>[]" <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab3); ?>" id="field_<?php echo($idindex); ?>_3">
            ...
            unset($disp_row);
                ...
                if ($cfg['LongtextDoubleTextarea'] && mb_strstr($type, 'longtext')) {
                    ?>
        ...
        <tr>
            <td colspan="4" align="right" bgcolor="<?php echo $bgcolor; ?>">
                ...
                <textarea name="fields<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" rows="<?php echo($cfg['TextareaRows'] * 2); ?>" cols="<?php echo($cfg['TextareaCols'] * 2); ?>" dir="<?php echo $text_dir; ?>" id="field_<?php echo($idindex); ?>_3"
                    <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab3); ?>"><?php echo $special_chars; ?></textarea>
            </td>
          <?php
                } else {
                    if (mb_strstr($type, 'text')) {
                        ?>
            <td bgcolor="<?php echo $bgcolor; ?>">
                <?php echo $backup_field . "\n"; ?>
            ...
            echo "\n" . '            ' . $backup_field;
                ...
                echo "\n";
                                ...
                                for ($j = 0; $j < $enum_cnt; $j++) {
                                    ...
                                    if ($data == $enum_atom
                        || ('' == $data && (!isset($primary_key) || 'YES' != $row_table_def['Null'])
                            && isset($row_table_def['Default']) && $enum_atom == $row_table_def['Default'])) {
                                        echo ' selected="selected"';
                                    }

                                    echo '>' . htmlspecialchars($enum_atom, ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
                                } // end for
                ...
                <?php
                        ...
                        || ('' == $data && (!isset($primary_key) || 'YES' != $row_table_def['Null'])
                            && isset($row_table_def['Default']) && $enum_atom == $row_table_def['Default'])) {
                                    ...
                                    echo '            <label for="field_' . ($tabindex + $tab3) . '_3_' . $j . '">' . htmlspecialchars($enum_atom, ENT_QUOTES | ENT_HTML5) . '</label>' . "\n";
                                } // end for
                            } // end else
            echo "\n"; ?>
            </td>
            ...
            <td bgcolor="<?php echo $bgcolor; ?>">
                ...
                <input type="hidden" name="fields<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" value="">
                <select name="field_<?php echo md5($field); ?><?php echo $vkey; ?>[]" size="<?php echo $size; ?>" multiple="multiple" <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab3); ?>" id="field_<?php echo($idindex); ?>_3">
            <?php
            echo "\n";
                                ...
                                for ($j = 0; $j < $countset; $j++) {
                                    ...
                                    echo '>' . htmlspecialchars($set[$j], ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
                                } // end for
            ?>
                </select>
            </td>
            ...
            <td bgcolor="<?php echo $bgcolor; ?>">
                <?php
                    echo $strBinaryDoNotEdit;
                                        ...
                                        if (isset($data)) {
                                            $data_size = PMA_formatByteDown(mb_strlen(stripslashes($data)), 3, 1);
            ...
            <td bgcolor="<?php echo $bgcolor; ?>">
                <?php echo $backup_field . "\n"; ?>
                <input type="text" name="fields<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" value="<?php echo $special_chars; ?>" size="<?php echo $fieldsize; ?>" maxlength="<?php echo $maxlength; ?>" class="textfield" <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab3); ?>" id="field_<?php echo($idindex); ?>_3">
                <?php
                                        }
                                    } // end if...elseif...else
...
'longblob' => '4294967296',
                                        ...
                                        if ($handle = @opendir($cfg['UploadDir'])) {
                                            ...
                                            while ($file = @readdir($handle)) {
                                                if (is_file($cfg['UploadDir'] . $file) && !PMA_checkFileExtensions($file, '.sql')) {
                                                    ...
                                                    echo '            <option value="' . htmlspecialchars($file, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($file, ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
            ...
            <td bgcolor="<?php echo $bgcolor; ?>">
                <?php echo $backup_field . "\n"; ?>
            <?php
            if ($is_char && isset($cfg['CharEditing']) && ('textarea' == $cfg['CharEditing'])) {
                echo "\n"; ?>
                ...
                <input type="text" name="fields<?php echo $vkey; ?>[<?php echo urlencode($field); ?>]" value="<?php echo $special_chars; ?>" size="<?php echo $fieldsize; ?>" maxlength="<?php echo $maxlength; ?>" class="textfield" <?php echo $chg_evtHandler; ?>="return unNullify('<?php echo urlencode($field); ?>', '<?php echo $jsvkey; ?>')" tabindex="<?php echo($tabindex + $tab3); ?>" id="field_<?php echo($idindex); ?>_3">
                <?php
                if ('date' == $type || 'datetime' == $type || 'timestamp' == mb_substr($type, 0, 9)) {
                    ?>
                    <script type="text/javascript">
                    <!--
                    document.write('<a title="<?php echo $strCalendar; ?>" href="javascript:openCalendar(\'<?php echo PMA_generate_common_url(); ?>\', \'insertForm\', \'field_<?php echo($idindex); ?>_3\', \'<?php echo mb_substr($type, 0, 9)?>\')"><img class="calendar" src="<?php echo $pmaThemeImage; ?>b_calendar.png" alt="<?php echo $strCalendar; ?>"></a>');
                    //-->
                    </script>
                    <?php
                }
...
if (isset($primary_key)) {
    ?>
            <input type="radio" name="submit_type" value="<?php echo $strSave; ?>" id="radio_submit_type_save" checked tabindex="<?php echo($tabindex + $tab3 + 1); ?>" style="vertical-align: middle"><label for="radio_submit_type_save"><?php echo $strSave; ?></label><br>
...
if (!empty($disp_message)) {
    $checked_after_insert_new_insert = ' checked';

    $checked_after_insert_back = '';
} else {
    $checked_after_insert_back = ' checked';
