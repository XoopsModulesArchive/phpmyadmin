...
{
    $tooltip_truename[$tmp['Name']] = ($GLOBALS['cfg']['ShowTooltipAliasTB'] && 'nested' != $GLOBALS['cfg']['ShowTooltipAliasTB'] ? (!empty($tmp['Comment']) ? $tmp['Comment'] . ' ' : $tmp['Name']) : $tmp['Name']);

    $tooltip_aliasname[$tmp['Name']] = ($GLOBALS['cfg']['ShowTooltipAliasTB'] && 'nested' != $GLOBALS['cfg']['ShowTooltipAliasTB'] ? $tmp['Name'] : (!empty($tmp['Comment']) ? $tmp['Comment'] . ' ' : $tmp['Name']));
...
if (true === $cfg['SkipLockedTables']) {
    ...
    if (false !== $db_info_result && PMA_DBI_num_rows($db_info_result) > 0) {
        while (false !== ($tmp = PMA_DBI_fetch_row($db_info_result))) {
...
if (!isset($sot_ready)) {
    ...
    if (false !== $db_info_result && PMA_DBI_num_rows($db_info_result) > 0) {
        while (false !== ($sts_tmp = PMA_DBI_fetch_assoc($db_info_result))) {
            if (!isset($sts_tmp['Type']) && isset($sts_tmp['Engine'])) {
                $sts_tmp['Type'] = &$sts_tmp['Engine'];
            }
