...
if (isset($submitorderby) && !empty($order_field)) {
    ...
    if (isset($order_order) && 'desc' == $order_order) {
        $sql_query .= ' DESC';
    }
...
?>
    ...
    <form method="post" action="tbl_rename.php" onsubmit="return emptyFormElements(this, 'new_name')">
        ...
        <tr>
            <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                <input type="text" size="20" name="new_name" value="<?php echo htmlspecialchars($table, ENT_QUOTES | ENT_HTML5); ?>" class="textfield" onfocus="this.select()">&nbsp;
            </td>
            <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>" align="right">
...
for ($i = 0; $i < $num_dbs; $i++) {
    ...
    echo '<option value="' . htmlspecialchars($dblist[$i], ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($dblist[$i], ENT_QUOTES | ENT_HTML5) . '</option>';
...
for ($i = 0; $i < $num_dbs; $i++) {
    ...
    echo '<option value="' . htmlspecialchars($dblist[$i], ENT_QUOTES | ENT_HTML5) . '"';

    if ($dblist[$i] == $db) {
        echo ' selected="selected"';
    }

    echo '>' . htmlspecialchars($dblist[$i], ENT_QUOTES | ENT_HTML5) . '</option>';
...
?>
        ...
        <tr>
            <td nowrap="nowrap" bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                ...
                <input type="radio" name="what" value="data" id="radio_copy_data" checked style="vertical-align: middle">
                ...
                <?php
                    ...
                    if (isset($_COOKIE) && isset($_COOKIE['pma_switch_to_new']) && 'true' == $_COOKIE['pma_switch_to_new']) {
                        $pma_switch_to_new = 'true';
                    }
                ?>
                <input type="checkbox" name="switch_to_new" value="true" id="checkbox_switch"<?php echo((isset($pma_switch_to_new) && 'true' == $pma_switch_to_new) ? ' checked' : ''); ?> style="vertical-align: middle">
...
?>
    ...
    <form method="post" action="tbl_properties_operations.php">
        <tr>
            <th colspan="2" class="tblHeaders" align="left">
                ...
                <?php $comment = preg_replace('; InnoDB free:.*$', '', preg_replace('^InnoDB free:.*$', '', $show_comment)); ?>
                <input type="hidden" name="prev_comment" value="<?php echo urlencode($comment); ?>">&nbsp;
            </th>
        <tr>
            <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                <input type="text" name="comment" maxlength="60" size="30" value="<?php echo htmlspecialchars($comment, ENT_QUOTES | ENT_HTML5); ?>" class="textfield" style="vertical-align: middle" onfocus="this.select()">&nbsp;
            </td>
            <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>" align="right">
        ...
        <tr>
            <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                <select name="tbl_type" style="vertical-align: middle">
                    <option value="MYISAM"<?php if ('MYISAM' == $tbl_type) {
        echo ' selected="selected"';
    } ?>>MyISAM</option>
                    <option value="HEAP"<?php if ('HEAP' == $tbl_type) {
        echo ' selected="selected"';
    } ?>>Heap</option>
    <?php
    $tbl_types = "\n";
    if (isset($tbl_bdb)) {
        $tbl_types .= '                <option value="BERKELEYDB"'
                   . (('BERKELEYDB' == $tbl_type) ? ' selected="selected"' : '')
                   . '>Berkeley DB</option>' . "\n";
    }
    if (isset($tbl_gemini)) {
        $tbl_types .= '                <option value="GEMINI"'
                   . (('GEMINI' == $tbl_type) ? ' selected="selected"' : '')
                   . '>Gemini</option>' . "\n";
    }
    if (isset($tbl_innodb)) {
        $tbl_types .= '                <option value="INNODB"'
                   . (('INNODB' == $tbl_type) ? ' selected="selected"' : '')
                   . '>INNO DB</option>' . "\n";
    }
    if (isset($tbl_isam)) {
        $tbl_types .= '                <option value="ISAM"'
                   . (('ISAM' == $tbl_type) ? ' selected="selected"' : '')
                   . '>ISAM</option>' . "\n";
    }

    echo $tbl_types;
    ?>
                    <option value="MERGE"<?php if ('MRG_MYISAM' == $tbl_type) {
        echo ' selected="selected"';
    } ?>>Merge</option>
                    </select>
            </td>
    ...
    if ('MYISAM' == $tbl_type || 'ISAM' == $tbl_type) {
        ?>
    <!-- Table options -->
    <form method="post" action="tbl_properties_operations.php">
        <tr>
        ...
        <tr>
            <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                ...
                <?php echo (isset($pack_keys) && 1 == $pack_keys) ? ' checked' : ''; ?> style="vertical-align: middle">
                <label for="pack_keys_opt">pack_keys</label><br>
        <?php
        if ('MYISAM' == $tbl_type) {
            ?>
                <input type="checkbox" name="checksum" id="checksum_opt"
                <?php echo (isset($checksum) && 1 == $checksum) ? ' checked' : ''; ?> style="vertical-align: middle">
                <label for="checksum_opt">checksum</label><br>

                <input type="checkbox" name="delay_key_write" id="delay_key_write_opt"
                <?php echo (isset($delay_key_write) && 1 == $delay_key_write) ? ' checked' : ''; ?> style="vertical-align: middle">
...
<?php
if ('MYISAM' == $tbl_type || 'BERKELEYDB' == $tbl_type || 'INNODB' == $tbl_type) {
    echo "\n";

    if ('MYISAM' == $tbl_type || 'INNODB' == $tbl_type) {
        ?>
    <tr>
        <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
    ...
    if ('MYISAM' == $tbl_type || 'BERKELEYDB' == $tbl_type) {
        ?>
    <tr>
        <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
...
?>
    <tr>
        <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                <a href="sql.php?<?php echo $url_query; ?>&amp;sql_query=<?php echo urlencode('FLUSH TABLE ' . PMA_backquote($table)); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strTableHasBeenFlushed, htmlspecialchars($table, ENT_QUOTES | ENT_HTML5))); if ($cfg['ShowTooltip']) {
    echo '&amp;reload=1';
} ?>">
