...
if (!empty($db_collation) && PMA_MYSQL_INT_VERSION >= 40101) {
    ...
    if (in_array($db_charset, $mysql_charsets, true) && in_array($db_collation, $mysql_collations[$db_charset], true)) {
 ...
 */
$message = $strDatabase . ' ' . htmlspecialchars($db, ENT_QUOTES | ENT_HTML5) . ' ' . $strHasBeenCreated;
require_once __DIR__ . '/' . $cfg['DefaultTabDatabase'];
