...
$is_gotofile = false;
if (isset($after_insert) && 'new_insert' == $after_insert) {
    $goto = 'tbl_change.php?'
...
} elseif (isset($after_insert) && 'same_insert' == $after_insert) {
    $goto = 'tbl_change.php?'
          . PMA_generate_common_url($db, $table, '&')
          . '&goto=' . urlencode($goto)
          . '&pos=' . $pos
...
if (isset($primary_key)) {
    ...
    $loop_array = (is_array($primary_key) ? $primary_key : [0 => $primary_key]);
...
foreach ($loop_array as $primary_key_index => $enc_primary_key) {
    ...
    foreach ($me_fields as $encoded_key => $val) {
        ...
        require __DIR__ . '/tbl_replace_fields.php';

        if (empty($me_funcs[$encoded_key])) {
            $cur_value = $val . ', ';
        } else {
            if (preg_match('@^(NOW|CURDATE|CURTIME|UNIX_TIMESTAMP|RAND|USER|LAST_INSERT_ID)$@', $me_funcs[$encoded_key])) {
    ...
    if ($is_insert) {
        if (empty($query)) {
            ...
            $query = ['INSERT INTO ' . PMA_backquote($table) . ' (' . $fieldlist . ') VALUES '];
        }
...
if ($is_gotofile) {
    if ('db_details.php' == $goto && !empty($table)) {
        unset($table);
    }
    ...
    $add_query = (!$seen_binary && mb_strlen($sql_query) < 1024 ? '&disp_query=' . urlencode($sql_query) : '');
