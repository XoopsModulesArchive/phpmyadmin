...
/* $Id: left.php,v 1.120 2003/05/02 19:56:44 garvinhicking Exp $ */
...
require __DIR__ . '/libraries/ob.lib.php';
if ($cfg['OBGzip']) {
...
require __DIR__ . '/libraries/bookmark.lib.php';
require __DIR__ . '/libraries/relation.lib.php';
...
<?php echo '<a href=' . XOOPS_URL . '/admin.php target=_top>Xoops</a><br>'; ?>
<?php
if ($cfg['LeftDisplayLogo']) {
    ?>
    <!-- phpMyAdmin logo -->
    <a href="http://wiki.xoops.org/wakka.php?wakka=phpMyAdmin" target="_blank"><img src="./images/pma_logo.png" width="88" height="31" border="0" alt="phpMyAdmin"></a>
    <?php
}
echo "\n";
?>
...
if ($num_dbs > 1) {
    ...
    for ($i = 0; $i < $num_dbs; $i++) {
        ...
        if ($cfg['ShowTooltip'] && PMA_MYSQL_INT_VERSION >= 32303
            ...
            $tooltip = [];
            $tooltip_name = [];
            $result  = PMA_$GLOBALS['xoopsDB']->queryF('SHOW TABLE STATUS FROM ' . PMA_backquote($db));
            while (false !== ($tmp = PMA_$GLOBALS['xoopsDB']->fetchBoth($result))) {
        ...
        <nobr><img name="imEx" src="images/minus.png" border="0" width="9" height="9" alt="-">
            ...
            echo "\n";
            ?>
        <a class="item" href="<?php echo $cfg['DefaultTabDatabase']; ?>?<?php echo $common_url_query; ?>" title="<?php echo ($db_tooltip != '' && $cfg['ShowTooltipAliasDB'] ? htmlspecialchars($db, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5)); ?>" onclick="if (capable) {expandBase('el<?php echo $j; ?>', false)}">
            <span class="heada"><?php echo ($db_tooltip != '' && $cfg['ShowTooltipAliasDB'] ? '<i>' . htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) . '</i>' : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)); ?><bdo dir="<?php echo($text_dir); ?>">&nbsp;&nbsp;</bdo></span><span class="headaCnt">(<?php echo $num_tables_disp; ?>)</span></a></nobr>
    </div>
    ...
    <div id="el<?php echo $j;?>Child" class="child" style="margin-bottom: 5px"<?php echo $on_mouse; ?>>
            ...
            for ($t = 0; $t < $num_tables; $t++) {
                ...
                $alias = (!empty($tooltip_name) && isset($tooltip_name[$table]))
                           ? htmlspecialchars($tooltip_name[$table], ENT_QUOTES | ENT_HTML5)
                           : '';
                $url_title = (!empty($tooltip) && isset($tooltip[$table]))
                           ? htmlspecialchars($tooltip[$table], ENT_QUOTES | ENT_HTML5)
                           : '';
        ...
        // list of tables contained in the current database
            ...
            if (!empty($db_start) && $db == $db_start) {
                ...
                for ($t = 0; $t < $num_tables; $t++) {
                    ...
                    $table_list .= '              <img src="images/browse.png" width="8" height="8" border="0" alt="' . $strBrowse . ': ' . $url_title . '" title="' . $strBrowse . ': ' . htmlspecialchars($url_title, ENT_QUOTES | ENT_HTML5) . '"></a><bdo dir="' . $text_dir . '">&nbsp;</bdo>' . "\n";
                    if (PMA_USR_BROWSER_AGENT == 'IE') {
                        $table_list .= '          <span class="tblItem"><a class="tblItem" id="tbl_' . md5($table) . '" title="' . htmlspecialchars($url_title, ENT_QUOTES | ENT_HTML5)
                                       . '" target="phpmain' . $hash . '" href="' . $cfg['DefaultTabTable'] . '?' . $common_url_query . '&amp;table=' . urlencode($table) . '">' . ($alias != '' && $cfg['ShowTooltipAliasTB'] ? $alias : htmlspecialchars($table, ENT_QUOTES | ENT_HTML5)) . '</a></span></nobr><br>' . "\n";
                    } else {
                        $table_list .= '          <a class="tblItem" id="tbl_' . md5($table) . '" title="' . htmlspecialchars($url_title, ENT_QUOTES | ENT_HTML5)
                                       . '" target="phpmain' . $hash . '" href="' . $cfg['DefaultTabTable'] . '?' . $common_url_query . '&amp;table=' . urlencode($table) . '">' . ($alias != '' && $cfg['ShowTooltipAliasTB'] ? $alias : htmlspecialchars($table, ENT_QUOTES | ENT_HTML5)) . '</a></nobr><br>' . "\n";
                    }
                } // end for $t (tables list)
                ...
                $table_list_header .= '        <span class="heada"><b>' . ($db_tooltip != '' && $cfg['ShowTooltipAliasTB'] ? htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)) . '</b><bdo dir="' . $text_dir . '">&nbsp;&nbsp;</bdo></span></a><br>' . "\n\n";
            } else {
                $selected = '';
            } // end if... else...

            if (!empty($num_tables)) {
                echo '            <option value="' . htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)
                     . '"' . $selected . '>' . ($db_tooltip != '' && $cfg['ShowTooltipAliasDB'] ? htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)) . ' (' . $num_tables . ')</option>' . "\n";
            } else {
                echo '            <option value="' . htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)
                     . '"' . $selected . '>' . ($db_tooltip != '' && $cfg['ShowTooltipAliasDB'] ? htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)) . ' (-)</option>' . "\n";
            } // end if... else...
...
// Case where only one database has to be displayed
    ...
    <div id="el2Parent" class="parent">
        <nobr><a class="item" href="<?php echo $cfg['DefaultTabDatabase']; ?>?<?php echo $common_url_query; ?>">
            <span class="heada"><?php echo ($db_tooltip != '' && $cfg['ShowTooltipAliasDB'] ? htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)); ?><bdo dir="<?php echo($text_dir); ?>">&nbsp;&nbsp;</bdo></span><span class="headaCnt">(<?php echo $num_tables_disp; ?>)</span></a></nobr>
    </div>
    ...
    for ($j = 0; $j < $num_tables; $j++) {
        ...
        <nobr><a target="phpmain<?php echo $hash; ?>" href="sql.php?<?php echo $common_url_query; ?>&amp;table=<?php echo urlencode($table); ?>&amp;sql_query=<?php echo (isset($book_sql_query) && $book_sql_query !== false ? urlencode($book_sql_query) : urlencode('SELECT * FROM ' . PMA_backquote($table))); ?>&amp;pos=0&amp;goto=<?php echo $cfg['DefaultTabTable']; ?>" title="<?php echo $strBrowse . ': ' . htmlspecialchars(
                  ...
                  <img src="images/browse.png" width="8" height="8" border="0" alt="<?php echo $strBrowse . ': ' . htmlspecialchars($table, ENT_QUOTES | ENT_HTML5); ?>"></a><bdo dir="<?php echo $text_dir; ?>">&nbsp;</bdo>
 ...
 */
if (isset($dbh) && $dbh) {
    @$GLOBALS['xoopsDB']->close($dbh);
}
if (isset($userlink) && $userlink) {
    @$GLOBALS['xoopsDB']->close($userlink);
}
