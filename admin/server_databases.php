...
function PMA_dbCmp($a, $b)
{
    ...
    if ('db_name' == $sort_by) {
        return ('asc' == $sort_order ? 1 : -1) * strcasecmp($a['db_name'], $b['db_name']);
    }
    ...
    return ('asc' == $sort_order ? 1 : -1) * ((int)$a[$sort_by] > (int)$b[$sort_by] ? 1 : -1);
...
if (count($statistics) > 0) {
    ...
    if (empty($dbstats)) {
        ...
        echo "\n"
           . '                <a href="./server_databases.php?' . $url_query . '&amp;dbstats=1&amp;sort_by=db_name&amp;sort_order=' . (('db_name' == $sort_by && 'asc' == $sort_order) ? 'desc' : 'asc') . '">' . "\n"
           . '                    ' . $strDatabase . "\n"
           . ('db_name' == $sort_by ? '                    <img src="' . $pmaThemeImage . 's_' . $sort_order . '.png" border="0" width="11" height="9"  alt="' . ('asc' == $sort_order ? $strAscending : $strDescending) . '">' . "\n" : '')
           . '                </a>' . "\n"
           . '                &nbsp;' . "\n"
           . '            </th>' . "\n";
        ...
        echo '            <th>' . "\n"
           . '                &nbsp;' . "\n"
           . '                <a href="./server_databases.php?' . $url_query . '&amp;dbstats=1&amp;sort_by=tbl_cnt&amp;sort_order=' . (('tbl_cnt' == $sort_by && 'desc' == $sort_order) ? 'asc' : 'desc') . '">' . "\n"
           . '                    ' . $strNumTables . "\n"
           . ('tbl_cnt' == $sort_by ? '                    <img src="' . $pmaThemeImage . 's_' . $sort_order . '.png" border="0" width="11" height="9"  alt="' . ('asc' == $sort_order ? $strAscending : $strDescending) . '">' . "\n" : '')
           . '                </a>' . "\n"
           . '                &nbsp;' . "\n"
           . '            </th>' . "\n"
           . '            <th colspan="2">' . "\n"
           . '                &nbsp;' . "\n"
           . '                <a href="./server_databases.php?' . $url_query . '&amp;dbstats=1&amp;sort_by=data_sz&amp;sort_order=' . (('data_sz' == $sort_by && 'desc' == $sort_order) ? 'asc' : 'desc') . '">' . "\n"
           . '                    ' . $strData . "\n"
           . ('data_sz' == $sort_by ? '                    <img src="' . $pmaThemeImage . 's_' . $sort_order . '.png" border="0" width="11" height="9"  alt="' . ('asc' == $sort_order ? $strAscending : $strDescending) . '">' . "\n" : '')
           . '                </a>' . "\n"
           . '                &nbsp;' . "\n"
           . '            </th>' . "\n"
           . '            <th colspan="2">' . "\n"
           . '                &nbsp;' . "\n"
           . '                <a href="./server_databases.php?' . $url_query . '&amp;dbstats=1&amp;sort_by=idx_sz&amp;sort_order=' . (('idx_sz' == $sort_by && 'desc' == $sort_order) ? 'asc' : 'desc') . '">' . "\n"
           . '                    ' . $strIndexes . "\n"
           . ('idx_sz' == $sort_by ? '                    <img src="' . $pmaThemeImage . 's_' . $sort_order . '.png" border="0" width="11" height="9"  alt="' . ('asc' == $sort_order ? $strAscending : $strDescending) . '">' . "\n" : '')
           . '                </a>' . "\n"
           . '                &nbsp;' . "\n"
           . '            </th>' . "\n"
           . '            <th colspan="2">' . "\n"
           . '                &nbsp;' . "\n"
           . '                <a href="./server_databases.php?' . $url_query . '&amp;dbstats=1&amp;sort_by=tot_sz&amp;sort_order=' . (('tot_sz' == $sort_by && 'desc' == $sort_order) ? 'asc' : 'desc') . '">' . "\n"
           . '                    ' . $strTotalUC . "\n"
           . ('tot_sz' == $sort_by ? '                    <img src="' . $pmaThemeImage . 's_' . $sort_order . '.png" border="0" width="11" height="9"  alt="' . ('asc' == $sort_order ? $strAscending : $strDescending) . '">' . "\n" : '')
           . '                </a>' . "\n"
           . '                &nbsp;' . "\n"
           . '            </th>' . "\n";
    }
...
'tot_sz' => 0,
    ...
    foreach ($statistics as $current) {
        [$data_size, $data_unit] = PMA_formatByteDown($current['data_sz'], 3, 1);

        [$idx_size, $idx_unit] = PMA_formatByteDown($current['idx_sz'], 3, 1);

        [$tot_size, $tot_unit] = PMA_formatByteDown($current['tot_sz'], 3, 1);
        ...
        if ($is_superuser || $cfg['AllowUserDropDatabase']) {
            echo '            <td bgcolor="' . ($useBgcolorOne ? $cfg['BgcolorOne'] : $cfg['BgcolorTwo']) . '">' . "\n"
               . '                <input type="checkbox" name="selected_db[]" title="' . htmlspecialchars($current['db_name'], ENT_QUOTES | ENT_HTML5) . '" value="' . htmlspecialchars($current['db_name'], ENT_QUOTES | ENT_HTML5) . '" ' . (empty($checkall) ? '' : 'checked ') . '>' . "\n"
               . '            </td>' . "\n";
        }

        echo '            <td bgcolor="' . ($useBgcolorOne ? $cfg['BgcolorOne'] : $cfg['BgcolorTwo']) . '">' . "\n"
           . '                <a onclick="reload_window(\'' . urlencode($current['db_name']) . '\'); return true;" href="' . $cfg['DefaultTabDatabase'] . '?' . $url_query . '&amp;db=' . urlencode($current['db_name']) . '" title="' . sprintf($strJumpToDB,
                                                                                                                                                                                                                                                 htmlspecialchars(
                                                                                                                                                                                                                                                     $current['db_name'],
                                                                                                                                                                                                                                                     ENT_QUOTES | ENT_HTML5
                                                                                                                                                                                                                                                 )
             ) . '">' . "\n"
           . '                    ' . htmlspecialchars($current['db_name'], ENT_QUOTES | ENT_HTML5) . "\n"
           . '                </a>' . "\n"
           . '            </td>' . "\n";
        ...
        if (!empty($dbstats)) {
            if (PMA_MYSQL_INT_VERSION >= 40101) {
                ...
                echo '            <td bgcolor="' . ($useBgcolorOne ? $cfg['BgcolorOne'] : $cfg['BgcolorTwo']) . '">' . "\n"
                   . '                <dfn title="' . htmlspecialchars(PMA_getCollationDescr($current_collation), ENT_QUOTES | ENT_HTML5) . '">' . "\n"
                   . '                    ' . htmlspecialchars($current_collation, ENT_QUOTES | ENT_HTML5) . "\n"
                   . '                </dfn>' . "\n"
                   . '            </td>' . "\n";
            }
        ...
        if ($is_superuser) {
            echo '            <td bgcolor="' . ($useBgcolorOne ? $cfg['BgcolorOne'] : $cfg['BgcolorTwo']) . '" align="center">' . "\n"
               . '                <a onclick="reload_window(\'' . urlencode($current['db_name']) . '\'); return true;" href="./server_privileges.php?' . $url_query . '&amp;checkprivs=' . urlencode($current['db_name']) . '" title="' . sprintf($strCheckPrivsLong,
                                                                                                                                                                                                                                                  htmlspecialchars(
                                                                                                                                                                                                                                                      $current['db_name'],
                                                                                                                                                                                                                                                      ENT_QUOTES | ENT_HTML5
                                                                                                                                                                                                                                                  )
    ...
    if (!empty($dbstats)) {
        [$data_size, $data_unit] = PMA_formatByteDown($total_calc['data_sz'], 3, 1);

        [$idx_size, $idx_unit] = PMA_formatByteDown($total_calc['idx_sz'], 3, 1);

        [$tot_size, $tot_unit] = PMA_formatByteDown($total_calc['tot_sz'], 3, 1);
