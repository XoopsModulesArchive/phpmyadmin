        ...
        for ($i = 0; $i < $num_dbs; $i++) {
            ...
            if ($cfg['ShowTooltip'] && PMA_MYSQL_INT_VERSION >= 32303
                ...
                while (false !== ($tmp = PMA_DBI_fetch_assoc($result))) {
                    ...
                    $tmp['Comment'] = ($cfg['ShowTooltipAliasTB'] && 'nested' !== $cfg['ShowTooltipAliasTB'] ? $tmp['Name'] : $tmp['Comment']);
            ...
            if (!empty($db_start) && $db == $db_start) {
                ...
                while (list($table) = PMA_DBI_fetch_row($tables)) {
                    ...
                    $url_title = (!empty($tooltip) && isset($tooltip[$table])) ? htmlentities($tooltip[$table], ENT_QUOTES | ENT_HTML5) : '';

                    $alias = (!empty($tooltip_name) && isset($tooltip_name[$table])) ? htmlentities($tooltip_name[$table], ENT_QUOTES | ENT_HTML5) : '';
            ...
            if (!empty($num_tables)) {
                echo '            '
                   . '<option value="' . htmlspecialchars($db, ENT_QUOTES | ENT_HTML5) . '"' . $selected . '>'
                   . ('' != $db_tooltip && $cfg['ShowTooltipAliasDB'] ? htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)) . ' (' . $num_tables . ')</option>' . "\n";
            } else {
                echo '            '
                   . '<option value="' . htmlspecialchars($db, ENT_QUOTES | ENT_HTML5) . '"' . $selected . '>'
                   . ('' != $db_tooltip && $cfg['ShowTooltipAliasDB'] ? htmlspecialchars($db_tooltip, ENT_QUOTES | ENT_HTML5) : htmlspecialchars($db, ENT_QUOTES | ENT_HTML5)) . ' (-)</option>' . "\n";
            } // end if... else...
        } // end for $i (db list)
        ?>
        </select>
