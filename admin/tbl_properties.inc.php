...
if (isset($selected) && is_array($selected)) {
    ...
    <input type="hidden" name="true_selected[] value="<?php echo($orig_field ?? urlencode($field)); ?>">
    <?php
}
...
for ($i = 0; $i < $num_fields; $i++) {
    ...
    if (isset($regenerate) && true === $regenerate) {
    ...
    if (preg_match('@^(set|enum)\((.+)\)$@i', $type, $tmp)) {
        ...
        $length = mb_substr(preg_replace('@([^,])\'\'@', '\\1\\\'', ',' . $tmp[2]), 1);
    } else {
        ...
        if (mb_strpos($type, '(')) {
            $length = rtrim(mb_substr($type, (mb_strpos($type, '(') + 1), (mb_strpos($type, ')') - mb_strpos($type, '(') - 1)));

            $type = rtrim(mb_substr($type, 0, mb_strpos($type, '(')));
        } else {
            $length = '';
        }
    } // end if else
    ...
    if (isset($submit_length) && false !== $submit_length) {
    ...
    for ($j = 0; $j < $cnt_column_types; $j++) {
        ...
        if (mb_strtoupper($type) == mb_strtoupper($cfg['ColumnTypes'][$j])) {
    ...
    if (isset($submit_attribute) && false !== $submit_attribute) {
    ...
    for ($j = 0; $j < $cnt_attribute_types; $j++) {
        ...
        if (mb_strtoupper($strAttribute) == mb_strtoupper($cfg['AttributeTypes'][$j])) {
            $content_cells[$i][$ci] .= ' selected="selected"';
        }
    ...
    if ((!isset($row) || empty($row['Null']) || 'NOT NULL' == $row['Null']) && false === $submit_null) {
        $content_cells[$i][$ci] .= "\n";
    ...
    if (!$is_backup) {
        if (isset($row) && isset($row['Key']) && 'PRI' == $row['Key']) {
            $checked_primary = ' checked';
        } else {
            $checked_primary = '';
        }

        if (isset($row) && isset($row['Key']) && 'MUL' == $row['Key']) {
            $checked_index = ' checked';
        } else {
            $checked_index = '';
        }

        if (isset($row) && isset($row['Key']) && 'UNI' == $row['Key']) {
        ...
        if ((isset($row) && isset($row['Comment']) && 'FULLTEXT' == $row['Comment'])) {
            $checked_fulltext = ' checked';
        } else {
            $checked_fulltext = '';
        }
    ...
    if ($cfgRelation['commwork']) {
        $content_cells[$i][$ci] = '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '" type="text" name="field_comments[]" size="12" value="' . (isset($row) && isset($row['Field']) && is_array($comments_map) && isset($comments_map[$row['Field']]) ? htmlspecialchars(
    ...
    if ($cfgRelation['mimework'] && $cfg['BrowseMIME'] && $cfgRelation['commwork']) {
        ...
        if (is_array($available_mime['mimetype'])) {
            foreach ($available_mime['mimetype'] as $mimekey => $mimetype) {
                ...
                $content_cells[$i][$ci] .= '    <option value="' . str_replace('/', '_', $mimetype) . '" ' . $checked . '>' . htmlspecialchars($mimetype, ENT_QUOTES | ENT_HTML5) . '</option>';
            }
        }
        ...
        if (is_array($available_mime['transformation'])) {
            foreach ($available_mime['transformation'] as $mimekey => $transform) {
                ...
                $tooltip = 'strTransformation_' . mb_strtolower(preg_replace('@(\.inc\.php3?)$@', '', $available_mime['transformation_file'][$mimekey]));

                $tooltip = $$tooltip ?? sprintf(str_replace('<br>', ' ', $strMIME_nodescription), 'PMA_transformation_' . $tooltip . '()');

                $content_cells[$i][$ci] .= '<option value="' . $available_mime['transformation_file'][$mimekey] . '" ' . $checked . ' title="' . htmlspecialchars($tooltip, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($transform, ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
            }
        }
        ...
        $content_cells[$i][$ci] = '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '" type="text" name="field_transformation_options[]" size="16" value="' . (isset($row) && isset($row['Field']) && isset($mime_map[$row['Field']]['transformation_options']) ? htmlspecialchars(
...
if ('horizontal' == $cfg['DefaultPropDisplay']) {
    ?>
...
if ('tbl_create.php' == $action) {
    ...
    <tr>
        <td>
            <input type="text" name="comment" size="40" maxlength="80" value="<?php echo($comment ?? ''); ?>" class="textfield">
        </td>
    <?php
    ...
    if ('tbl_create.php' == $action) {
        ...
        if ($result) {
            while (false !== ($tmp = PMA_DBI_fetch_assoc($result))) {
                if (isset($tmp['Variable_name'])) {
                    switch ($tmp['Variable_name']) {
                        case 'have_bdb':
                            if (isset($tmp['Variable_name']) && 'YES' == $tmp['Value']) {
                                $tbl_bdb = true;
                            }
                            break;
                        case 'have_gemini':
        ...
        <td>
            <select name="tbl_type">
                <option <?php echo(isset($tbl_type) && 'Default' == $tbl_type ? 'selected="checked"' : ''); ?> value="Default"><?php echo $strDefault; ?></option>
                <option <?php echo(isset($tbl_type) && 'MYISAM' == $tbl_type ? 'selected="checked"' : ''); ?> value="MYISAM">MyISAM</option>
                <option <?php echo(isset($tbl_type) && 'HEAP' == $tbl_type ? 'selected="checked"' : ''); ?> value="HEAP">Heap</option>
                <option <?php echo(isset($tbl_type) && 'MERGE' == $tbl_type ? 'selected="checked"' : ''); ?> value="MERGE">Merge</option>
                <?php if (isset($tbl_bdb)) { ?><option <?php echo(isset($tbl_type) && 'BDB' == $tbl_type ? 'selected="checked"' : ''); ?> value="BDB">Berkeley DB</option><?php } ?>
                <?php if (isset($tbl_gemini)) { ?><option <?php echo(isset($tbl_type) && 'GEMINI' == $tbl_type ? 'selected="checked"' : ''); ?> value="GEMINI">Gemini</option><?php } ?>
                <?php if (isset($tbl_innodb)) { ?><option <?php echo(isset($tbl_type) && 'INNO DB' == $tbl_type ? 'selected="checked"' : ''); ?> value="InnoDB">INNO DB</option><?php } ?>
                <?php if (isset($tbl_isam)) { ?><option <?php echo(isset($tbl_type) && 'ISAM' == $tbl_type ? 'selected="checked"' : ''); ?> value="ISAM">ISAM</option><?php } ?>
            </select>
        </td>
        <?php
        if (PMA_MYSQL_INT_VERSION >= 40100) {
...
<?php
if ('tbl_create.php' == $action || 'tbl_addfield.php' == $action) {
