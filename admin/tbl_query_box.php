...
/* $Id: tbl_query_box.php,v 2.19 2004/06/27 12:05:16 lem9 Exp $ */
...
if (!($cfg['QueryFrame'] && $cfg['QueryFrameJS'] && isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && ('sql' == $querydisplay_tab || 'full' == $querydisplay_tab))) {
    PMA_checkParameters(['db', 'table', 'url_query']);
}

/**
 * Defines the query to be displayed in the query textarea
 */
if (isset($show_query) && '1' == $show_query) {
    // This script has been called by read_dump.php

    $query_to_display = $sql_query_cpy ?? $sql_query;
} else {
    $query_to_display = '';
}
unset($sql_query);
...
if ($cfg['QueryFrame'] && (!$cfg['QueryFrameJS'] && !$db || ($cfg['QueryFrameJS'] && !$db))) {
    if ($num_dbs > 0) {
        ...
        for ($i = 0; $i < $num_dbs; $i++) {
            ...
            $queryframe_db_list .= '<option value="' . htmlspecialchars($t_db, ENT_QUOTES | ENT_HTML5) . '">' . htmlspecialchars($t_db, ENT_QUOTES | ENT_HTML5) . '</option>';
        }
...
if ($cfg['QueryFrame'] && $cfg['QueryFrameJS'] && isset($is_inside_querywindow) && $is_inside_querywindow) {
    ?>
        ...
        <!--
        document.writeln('<form method="post" target="phpmain' +  <?php echo((isset($is_inside_querywindow) && true === $is_inside_querywindow) ? 'opener.' : ''); ?>parent.frames.queryframe.document.hashform.hash.value + '" action="read_dump.php"<?php if ($is_upload) {
...
if (!isset($is_inside_querywindow) ||
    (isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && ('sql' == $querydisplay_tab || 'full' == $querydisplay_tab))) {
    ?>
    <!-- Query box and bookmark support -->
<?php
    if (isset($is_inside_querywindow) && true === $is_inside_querywindow) {
        $querybox_hidden_fields .= '                    <input type="hidden" name="focus_querywindow" value="true">' . "\n";
    } ?>
            <tr class="tblHeaders">
                <td nowrap="nowrap"<?php if ('' == $queryframe_tdcolspan) {
        echo ' colspan="2"';
    } ?>>
                    <a name="querybox"></a>
                    <?php echo sprintf($strRunSQLQuery, htmlspecialchars($db, ENT_QUOTES | ENT_HTML5) . $queryframe_db_list) . PMA_showMySQLDocu('Reference', 'SELECT'); ?>
                </td>
                <?php
    if (isset($table) && $fields_cnt > 0) { ?>
            ...
            <tr bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                <td valign="top"<?php if ('' == $queryframe_tdcolspan) {
        echo ' colspan="2"';
    } ?>>
                    <textarea name="sql_query" rows="<?php echo $cfg['TextareaRows']; ?>" cols="<?php echo((isset($is_inside_querywindow) && true === $is_inside_querywindow && '' != $queryframe_tdcolspan) ? ceil($cfg['TextareaCols'] * 1.25) : $cfg['TextareaCols'] * 2); ?>" dir="<?php echo $text_dir; ?>"<?php echo $auto_sel; ?>><?php
    if (!empty($query_to_display)) {
        echo htmlspecialchars($query_to_display, ENT_QUOTES | ENT_HTML5);
    } elseif (isset($table)) {
        echo htmlspecialchars(str_replace('%d', PMA_backquote($db), str_replace('%t', PMA_backquote($table), $cfg['DefaultQueryTable'])), ENT_QUOTES | ENT_HTML5);
    } else {
        echo htmlspecialchars(str_replace('%d', PMA_backquote($db), $cfg['DefaultQueryDatabase']), ENT_QUOTES | ENT_HTML5);
    } ?></textarea>
                </td>
<?php
    if (isset($table) && $fields_cnt > 0) {
...
<?php
        ...
        for ($i = 0; $i < $fields_cnt; $i++) {
            echo '                        '
               . '<option value="' . PMA_backquote(htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5)) . '">' . htmlspecialchars($fields_list[$i], ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
        } ?>
                    </select>        
                </td>
<?php
            ...
            <tr bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
                <td<?php if ('' != $queryframe_tdcolspan) {
        echo ' colspan="2"';
    } //echo $queryframe_tdcolspan;?>>
                    <input type="checkbox" name="show_query" value="1" id="checkbox_show_query" checked>
...
if ($is_upload && (!isset($is_inside_querywindow) ||
    (isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && ('files' == $querydisplay_tab || 'full' == $querydisplay_tab)))) {
    $form_items++; ?>
            <tr>
...
<?php
    echo '            '
       . ((isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && 'full' == $querydisplay_tab) || !isset($is_inside_querywindow) ? '<i>' . $strOr . '</i>' : '')
       . ' ' . $strLocationTextfile . ':&nbsp;' . "\n"; ?>
                </td>
            </tr>
...
<?php
    ...
    if ($is_bzip || $is_gzip) {
        echo '                </td>' . "\n"
           ...
           . '                    &nbsp;&nbsp;' . $strCompression . ':<br>&nbsp;&nbsp;' . "\n"
           . '                    &nbsp;&nbsp;<input type="radio" id="radio_sql_file_compression_auto" name="sql_file_compression" value="" checked>'
...
if (!empty($cfg['UploadDir']) && !isset($is_inside_querywindow) ||
    (!empty($cfg['UploadDir']) && isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && ('files' == $querydisplay_tab || 'full' == $querydisplay_tab))) {
...
$bookmark_go = false;
if (!isset($is_inside_querywindow) ||
    (isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && ('history' == $querydisplay_tab || 'full' == $querydisplay_tab))) {
    if ($cfg['Bookmark']['db'] && $cfg['Bookmark']['table']) {
        if (($bookmark_list = PMA_listBookmarks($db, $cfg['Bookmark'])) && count($bookmark_list) > 0) {
            ...
            echo '            ' . ((isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && 'full' == $querydisplay_tab) || !isset($is_inside_querywindow) ? "<i>$strOr</i>" : '') . " $strBookmarkQuery:&nbsp;\n";
            ...
            foreach ($bookmark_list as $key => $value) {
                echo '                <option value="' . $value . '">' . htmlspecialchars($key, ENT_QUOTES | ENT_HTML5) . '</option>' . "\n";
            }
            ...
            echo '            <input type="radio" name="action_bookmark" value="0" id="radio_bookmark0" checked style="vertical-align: middle"><label for="radio_bookmark0">' . $strSubmit . '</label>' . "\n";
...
if (!isset($is_inside_querywindow) || (isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && (('files' == $querydisplay_tab) || 'sql' == $querydisplay_tab || 'full' == $querydisplay_tab || ('history' == $querydisplay_tab && $bookmark_go)))) {
    if ($form_items > 0) {
        ?><!-- not needed 
            <table border="0">
                <tr>
...
<?php
        if ($cfg['Bookmark']['db']
          && $cfg['Bookmark']['table'] && (!isset($is_inside_querywindow)
            || (isset($is_inside_querywindow)
               && true === $is_inside_querywindow && isset($querydisplay_tab)
               //&& $querydisplay_tab != 'history'))) {
               && 'sql' == $querydisplay_tab))) {
            ?>
...
if (!isset($is_inside_querywindow) ||
    (isset($is_inside_querywindow) && true === $is_inside_querywindow && isset($querydisplay_tab) && ('files' == $querydisplay_tab || 'full' == $querydisplay_tab)) && isset($db) && '' != $db) {
    // loic1: displays import dump feature only if file upload available

    $ldi_target = 'ldi_table.php?' . $url_query . (isset($is_inside_querywindow) && true === $is_inside_querywindow ? '&amp;focus_querywindow=true' : '');
    ...
    if ($is_upload && isset($db) && isset($table)) {
        ...
        <!-- Insert a text file -->
            ...
            <script type="text/javascript">
                <!--
                document.writeln('<a href="<?php echo(isset($is_inside_querywindow) && true === $is_inside_querywindow ? '#' : $ldi_target); ?>" <?php echo(isset($is_inside_querywindow) && true === $is_inside_querywindow ? 'onclick="opener.parent.frames.phpmain\' + opener.parent.frames.queryframe.document.hashform.hash.value + \'.location.href = \\\'' . $ldi_target . '\\\'; return false;"' : ''); ?>><?php echo addslashes($imgInsertTextfiles . $strInsertTextfiles); ?></a>');
                //-->
            </script>
            ...
            <script type="text/javascript">
                <!--
                document.writeln('<a href="<?php echo(isset($is_inside_querywindow) && true === $is_inside_querywindow ? '#' : $ldi_target); ?>" <?php echo(isset($is_inside_querywindow) && true === $is_inside_querywindow ? 'onclick="opener.parent.frames.phpmain' . md5($cfg['PmaAbsoluteUri']) . '.location.href = \\\'' . $ldi_target . '\\\'; return false;"' : ''); ?>><?php echo addslashes($imgInsertTextfiles . $strInsertTextfiles); ?></a>');
                //-->
            </script>
            ...
            <noscript>
               <a href="<?php echo $ldi_target; ?>"><?php
                  echo $imgInsertTextfiles . $strInsertTextfiles; ?></a>
            </noscript>
        <?php
        //if (!isset($is_inside_querywindow) || !$is_inside_querywindow) echo "</li>\n";
    }
