...
if (!isset($err_url)) {
    $err_url = (!empty($back) ? $back : $goto)
             ...
             . ((1 != mb_strpos(' ' . $goto, 'db_details') && isset($table)) ? '&amp;table=' . urlencode($table) : '');
} // end if
...
PMA_checkParameters(['sql_query', 'db']);
...
if (isset($store_bkm)) {
    ...
    PMA_addBookmarks($fields, $cfg['Bookmark'], (isset($bkm_all_users) && 'true' == $bkm_all_users ? true : false));
...
if (isset($btnDrop) && $btnDrop == $strNo) {
    ...
    if ($is_gotofile) {
        if (1 == mb_strpos(' ' . $goto, 'db_details') && !empty($table)) {
            unset($table);
        }

        $active_page = $goto;

        require __DIR__ . '/' . PMA_securePath($goto);
    } else {
 ...
 */
if (!$cfg['Confirm']
    || (isset($is_js_confirmed) && $is_js_confirmed) || isset($btnDrop)
...
if ($do_confirm) {
    ...
    echo $strDoYouReally . "\n"
       ...
       . '    <tr>' . "\n"
       . '        <td class="tblError">' . "\n"
       . '            <tt>' . htmlspecialchars($stripped_sql_query, ENT_QUOTES | ENT_HTML5) . '</tt>&nbsp;?<br>' . "\n"
       . '        </td>' . "\n"
       . '    </tr>' . "\n"
    ...
    <input type="hidden" name="reload" value="<?php echo $reload ?? 0; ?>">
    <input type="hidden" name="purge" value="<?php echo $purge ?? ''; ?>">
    <input type="hidden" name="cpurge" value="<?php echo $cpurge ?? ''; ?>">
    <input type="hidden" name="purgekey" value="<?php echo $purgekey ?? ''; ?>">
    <input type="hidden" name="show_query" value="<?php echo $show_query ?? ''; ?>">
 ...
 */
    ...
    if ($is_select) { // see line 141
        ...
        if (preg_match('@^EXPLAIN[[:space:]]+@i', $sql_query)) {
            $is_explain = true;
        } else {
            if (preg_match('@^DELETE[[:space:]]+@i', $sql_query)) {
                $is_delete = true;

                $is_affected = true;
            } else {
                if (preg_match('@^(INSERT|LOAD[[:space:]]+DATA|REPLACE)[[:space:]]+@i', $sql_query)) {
                    $is_insert = true;

                    $is_affected = true;
                } else {
                    if (preg_match('@^UPDATE[[:space:]]+@i', $sql_query)) {
                        $is_affected = true;
                    } else {
                        if (preg_match('@^SHOW[[:space:]]+@i', $sql_query)) {
                            $is_show = true;
                        } else {
                            if (preg_match('@^(CHECK|ANALYZE|REPAIR|OPTIMIZE)[[:space:]]+TABLE[[:space:]]+@i', $sql_query)) {
                                $is_maint = true;
                            }
                        }
                    }
    ...
    if (isset($pos)
        && (!$cfg['ShowAll'] || 'all' != $session_max_rows) && !($is_count || $is_export || $is_func || $is_analyse) && isset($analyzed_sql[0]['queryflags']['select_from'])
    ...
    if ($is_delete
        ...
        if ($cnt_all_result) {
            [$num_rows] = PMA_DBI_fetch_row($cnt_all_result);
    ...
    if (isset($GLOBALS['show_as_php']) || !empty($GLOBALS['validatequery'])) {
        ...
        if ($error = PMA_DBI_getError()) {
            ...
            $full_err_url = (preg_match('@^(db_details|tbl_properties)@', $err_url)) ? $err_url . '&amp;show_query=1&amp;sql_query=' . urlencode($sql_query) : $err_url;
        ...
        // $GLOBALS['xoopsDB']->getAffectedRows() reports about the last query done)
        ...
        if (empty($sql_limit_to_append)) {
            ...
            if ($is_select) {
                ...
                if (!$is_group
                 && !isset($analyzed_sql[0]['queryflags']['union']) && !isset($analyzed_sql[0]['table_ref'][1]['table_name']) && (empty($analyzed_sql[0]['where_clause'])
                   || '1 ' == $analyzed_sql[0]['where_clause'])) {
                    ...
                    if (PMA_MYSQL_INT_VERSION < 40000) {
                        ...
                        if (mb_stristr($sql_query, 'DISTINCT')) {
                    ...
                    if (PMA_MYSQL_INT_VERSION < 40000
                     && '*' == $count_what && (!empty($analyzed_sql[0]['group_by_clause'])
                    ...
                    if (PMA_MYSQL_INT_VERSION < 40000) {
                        ...
                        [$unlim_num_rows] = PMA_DBI_fetch_row($cnt_all_result);
        ...
        if (isset($cpurge) && '1' == $cpurge && isset($purgekey)
    ...
    if ($num_rows < 1 || $is_affected) {
        ...
        if ($is_gotofile) {
            ...
            if (1 == mb_strpos(' ' . $goto, 'tbl_properties')) {
                if (!isset($table)) {
                    $goto = 'db_details.php';
                } else {
            ...
            if (1 == mb_strpos(' ' . $goto, 'db_details')) {
                if (isset($table)) {
                    unset($table);
                }
            ...
            if (1 == mb_strpos(' ' . $goto, 'db_details')
                || 1 == mb_strpos(' ' . $goto, 'tbl_properties')) {
    ...
    if (isset($printview) && '1' == $printview) {
        require_once __DIR__ . '/header_printview.inc.php';
    } else {
        $js_to_run = 'functions.js';
    ...
    if (isset($disp_query) && true === $cfg['ShowSQL']) {
    ...
    require_once __DIR__ . '/libraries/display_tbl.lib.php';
    ...
    if ('1' == $disp_mode[6] || '1' == $disp_mode[9]) {
        echo "\n";
        ...
        if ('1' == $disp_mode[9]) {
            ...
            echo '    <!-- Print view -->' . "\n"
                   ...
                   . ((isset($dontlimitchars) && '1' == $dontlimitchars) ? '&amp;dontlimitchars=1' : '')
                   . '" target="print_view">'
    ...
    if ('SELECT' == $analyzed_sql[0]['querytype'] && !isset($printview)) {
...
<tr bgcolor="<?php echo $cfg['BgcolorOne']; ?>"><td>
    ...
    <input type="hidden" name="goto" value="<?php echo $goto; ?>">
    <input type="hidden" name="fields[dbase]" value="<?php echo htmlspecialchars($db, ENT_QUOTES | ENT_HTML5); ?>">
    <input type="hidden" name="fields[user]" value="<?php echo $cfg['Bookmark']['user']; ?>">
    <input type="hidden" name="fields[query]" value="<?php echo urlencode($complete_query ?? $sql_query); ?>">
        </td><td>
