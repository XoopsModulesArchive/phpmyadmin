...
if (isset($btnLDI) && empty($textfile)) {
    ...
    require __DIR__ . '/ldi_table.php';
} elseif (isset($btnLDI) && ('none' != $textfile)) {
    if (!isset($replace)) {
        $replace = '';
    }
    ...
    if (isset($enclose_option) && mb_strlen($enclose_option) > 0) {
        $sql_query .= ' OPTIONALLY';
    }

    if (mb_strlen($enclosed) > 0) {
        $sql_query .= ' ENCLOSED BY \'' . $enclosed . '\'';
    }

    if (mb_strlen($escaped) > 0) {
        $sql_query .= ' ESCAPED BY \'' . $escaped . '\'';
    }

    if (mb_strlen($line_terminator) > 0) {
        $sql_query .= ' LINES TERMINATED BY \'' . $line_terminator . '\'';
    }

    if (mb_strlen($column_name) > 0) {
        $sql_query .= ' (';

        $tmp = preg_split(',( ?)', $column_name);
