...
function PMA_auth_check()
{
    ...
    if (empty($PHP_AUTH_PW)) {
        if (!empty($_SERVER) && isset($_SERVER['PHP_AUTH_PW'])) {
            ...
            if (isset($REMOTE_PASSWORD)) {
                $PHP_AUTH_PW = $REMOTE_PASSWORD;
            } else {
                if (!empty($_ENV) && isset($_ENV['REMOTE_PASSWORD'])) {
                    $PHP_AUTH_PW = $_ENV['REMOTE_PASSWORD'];
                } else {
                    if (@getenv('REMOTE_PASSWORD')) {
                    ...
                    // Fix from Matthias Fichtner for WebSite Professional - Part 2
                        ...
                        if (isset($AUTH_PASSWORD)) {
                            ...
                            if (!empty($_ENV) && isset($_ENV['AUTH_PASSWORD'])) {
                                $PHP_AUTH_PW = $_ENV['AUTH_PASSWORD'];
                            } else {
                                if (@getenv('AUTH_PASSWORD')) {
    ...
    if (empty($PHP_AUTH_USER) && empty($PHP_AUTH_PW)
        ...
        if (!empty($HTTP_AUTHORIZATION)
            && 'Basic ' == mb_substr($HTTP_AUTHORIZATION, 0, 6)) {
            [$PHP_AUTH_USER, $PHP_AUTH_PW] = explode(':', base64_decode(mb_substr($HTTP_AUTHORIZATION, 6), true));
        } else {
            if (!empty($_ENV)
             && isset($_ENV['HTTP_AUTHORIZATION'])
             && 'Basic ' == mb_substr($_ENV['HTTP_AUTHORIZATION'], 0, 6)) {
                [$PHP_AUTH_USER, $PHP_AUTH_PW] = explode(':', base64_decode(mb_substr($_ENV['HTTP_AUTHORIZATION'], 6), true));
            } else {
                if (@getenv('HTTP_AUTHORIZATION')
                 && 'Basic ' == mb_substr(getenv('HTTP_AUTHORIZATION'), 0, 6)) {
                    [$PHP_AUTH_USER, $PHP_AUTH_PW] = explode(':', base64_decode(mb_substr(getenv('HTTP_AUTHORIZATION'), 6), true));
                }
            }
        }
    } // end IIS
