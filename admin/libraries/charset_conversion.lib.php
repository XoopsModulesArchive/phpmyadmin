 ...
 * conversion function.
 *
 * @param mixed $src_charset
 * @param mixed $dest_charset
 * @param mixed $what
 *
 * @return  string   converted text
 *
...
function PMA_convert_file($src_charset, $dest_charset, $file)
{
    switch ($GLOBALS['PMA_recoding_engine']) {
        ...
        case PMA_CHARSET_LIBICONV:
            ...
            if (PMA_CHARSET_RECODE == $GLOBALS['PMA_recoding_engine']) {
                recode_file($src_charset . '..' . $dest_charset, $fin, $fout);
            } else {
                while (!feof($fin)) {
                    $line = fgets($fin, 4096);

                    if (PMA_CHARSET_ICONV == $GLOBALS['PMA_recoding_engine']) {
