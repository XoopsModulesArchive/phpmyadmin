 ...
 * @param mixed $the_disp_mode
 * @param mixed $the_total
 */
 ...
 *
 * @param  string  $the_disp_mode   the synthetic value for display_mode (see §1 a few
 *                   lines above for explanations)
 * @param  int  $the_total  the total number of rows returned by the sql query
 ...
 * @global  int  the total number of rows returned by the sql query
 *                   without any programmatically appended "LIMIT" clause
...
function PMA_setDisplayMode(&$the_disp_mode, &$the_total)
{
    ...
    if ('nnnn000000' != $the_disp_mode) {
        // 2.0 Print view -> set all elements to FALSE!

        if (isset($GLOBALS['printview']) && '1' == $GLOBALS['printview']) {
        ...
        //     contains a "PROC ANALYSE" part
            ...
            // 2.2 Statement is a "SHOW..."
                ...
                if ($GLOBALS['is_show']) {
                    ...
                    if (isset($which[1]) && mb_strpos(' ' . mb_strtoupper($which[1]), 'PROCESSLIST') > 0) {
                        $do_display['edit_lnk'] = 'nn'; // no edit link
                ...
                //     $do_display['text_btn'] (keeps other default values)
                    ...
                    for ($i = 0; $i < $GLOBALS['fields_cnt']; $i++) {
                        $is_link = ('nn' != $do_display['edit_lnk']
                            || 'nn' != $do_display['del_lnk'] || '0' != $do_display['sort_lnk']
                            || '0' != $do_display['ins_row']);
    ...
    if (isset($unlim_num_rows) && '' != $unlim_num_rows) {
        ...
        if (('1' == $do_display['nav_bar'] || '1' == $do_display['sort_lnk'])
    ...
    $the_disp_mode = implode('', $do_display);
 ...
 *
 * @param mixed $pos_next
 * @param mixed $pos_prev
 * @param mixed $encoded_query
 *
 * @global  string   the current language
 * @global  string   the currect charset for MySQL
 * @global  int  the server to use (refers to the number in the
 *                   configuration file)
 * @global  string   the database name
 * @global  string   the table name
 ...
 * @global  int  the total number of rows returned by the sql query
 *                   without any programmatically appended "LIMIT" clause
 * @global  int  the current position in results
 ...
 * @global  int  the number of row to display between two table headers
 * @global  bool  whether to limit the number of displayed characters of
...
<td align="center">
    <form action="sql.php" method="post"
        ...
        <input type="text" name="session_max_rows" size="3" value="<?php echo(('all' != $session_max_rows) ? $session_max_rows : $GLOBALS['cfg']['MaxRows']); ?>" class="textfield" onfocus="this.select()">
    ...
    $param1 = '            <select name="disp_direction">' . "\n"
            . '                <option value="horizontal"' . (('horizontal' == $disp_direction) ? ' selected="selected"' : '') . '>' . $GLOBALS['strRowsModeHorizontal'] . '</option>' . "\n"
            . '                <option value="horizontalflipped"' . (('horizontalflipped' == $disp_direction) ? ' selected="selected"' : '') . '>' . $GLOBALS['strRowsModeFlippedHorizontal'] . '</option>' . "\n"
            . '                <option value="vertical"' . (('vertical' == $disp_direction) ? ' selected="selected"' : '') . '>' . $GLOBALS['strRowsModeVertical'] . '</option>' . "\n"
            . '            </select>' . "\n"
            . '           ';
 ...
 * @param mixed $fields_meta
 * @param mixed $fields_cnt
 * @param mixed $analyzed_sql
 *
 ...
 * @global  int  the total number of rows returned by the sql query
 * @global  int  the current position in results
 * @global  int  the maximum number of rows per page
...
function PMA_displayTableHeaders($is_display, $fields_meta, $fields_cnt = 0, $analyzed_sql = '')
{
    ...
    if ('1' == $is_display['sort_lnk']) {
        ...
        if (isset($analyzed_sql) && isset($analyzed_sql[0]) &&
            isset($analyzed_sql[0]['querytype']) && 'SELECT' == $analyzed_sql[0]['querytype'] &&
            isset($analyzed_sql[0]['table_ref']) && 1 == count($analyzed_sql[0]['table_ref'])) {
            ...
            if (isset($indexes_data)) {
                if ('horizontal' == $disp_direction || 'horizontalflipped' == $disp_direction) {
                    $span = $fields_cnt;

                    if ('nn' != $is_display['edit_lnk']) {
                        $span++;
                    }

                    if ('nn' != $is_display['del_lnk']) {
                        $span++;
                    }

                    if ('kp' != $is_display['del_lnk'] && 'nn' != $is_display['del_lnk']) {
                        $span++;
                    }
                } else {
                ...
                foreach ($indexes_data as $key => $val) {
                    ...
                    $asc_sort = mb_substr($asc_sort, 0, -3);

                    $desc_sort = mb_substr($desc_sort, 0, -3);

                    $used_index = $used_index || $local_order == $asc_sort || $local_order == $desc_sort;

                    echo '<option value="' . htmlspecialchars($unsorted_sql_query . ' ORDER BY ' . $asc_sort, ENT_QUOTES | ENT_HTML5)
                         . '"' . ($local_order == $asc_sort ? ' selected="selected"' : '') . '>' . htmlspecialchars($key, ENT_QUOTES | ENT_HTML5)
                         . ' (' . $GLOBALS['strAscending'] . ')</option>';

                    echo "\n";

                    echo '<option value="' . htmlspecialchars($unsorted_sql_query . ' ORDER BY ' . $desc_sort, ENT_QUOTES | ENT_HTML5)
                         . '"' . ($local_order == $desc_sort ? ' selected="selected"' : '') . '>' . htmlspecialchars($key, ENT_QUOTES | ENT_HTML5)
                         . ' (' . $GLOBALS['strDescending'] . ')</option>';

                    echo "\n";
                }

                echo '<option value="' . htmlspecialchars($unsorted_sql_query, ENT_QUOTES | ENT_HTML5) . '"' . ($used_index ? '' : ' selected="selected"') . '>' . $GLOBALS['strNone'] . '</option>';
    ...
    if ('dr' == $is_display['del_lnk'] || 'kp' == $is_display['del_lnk']) {
...
<tr>
    ...
    if ('horizontal' == $disp_direction || 'horizontalflipped' == $disp_direction) {
        $colspan = ('nn' != $is_display['edit_lnk'] && 'nn' != $is_display['del_lnk']) ? ' colspan="3"' : '';
    } else {
        $rowspan = ('nn' != $is_display['edit_lnk'] && 'nn' != $is_display['del_lnk']) ? ' rowspan="3"' : '';
    }
    ...
    if (('nn' == $is_display['edit_lnk'] && 'nn' == $is_display['del_lnk'])
        && '1' == $is_display['text_btn']) {
        $vertical_display['emptypre'] = ('nn' != $is_display['edit_lnk'] && 'nn' != $is_display['del_lnk']) ? 3 : 0;
...
<th class="td" colspan="<?php echo $num_rows + floor($num_rows / $repeat_cells) + 1; ?>" align="center">
    ...
    //     and required
        ...
        if ($GLOBALS['cfg']['ModifyDeleteAtLeft'] && '1' == $is_display['text_btn']) {
            $vertical_display['emptypre'] = ('nn' != $is_display['edit_lnk'] && 'nn' != $is_display['del_lnk']) ? 3 : 0;
    ...
    <a href="<?php echo $text_url; ?>">
        ...
        //     ... else if no button, displays empty(ies) col(s) if required
            ...
            if ($GLOBALS['cfg']['ModifyDeleteAtLeft']
             && ('nn' != $is_display['edit_lnk'] || 'nn' != $is_display['del_lnk'])) {
                $vertical_display['emptypre'] = ('nn' != $is_display['edit_lnk'] && 'nn' != $is_display['del_lnk']) ? 3 : 0;
...
<td<?php echo $colspan; ?>></td>
    ...
    if ($GLOBALS['cfg']['ShowBrowseComments'] && $GLOBALS['cfgRelation']['commwork'] && 'horizontalflipped' != $disp_direction) {
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        ...
        if (isset($comments_map[$fields_meta[$i]->name])) {
            ...
            $comments_table_wrap_post = '<span class="tblcomment">' . htmlspecialchars($comments_map[$fields_meta[$i]->name], ENT_QUOTES | ENT_HTML5) . '</span>';
        } else {
            $comments_table_wrap_pre = '';
        ...
        if ('1' == $is_display['sort_lnk']) {
            ...
            if (($is_join
                && !preg_match('~([^[:space:],]|`[^`]`)[[:space:]]+(as[[:space:]]+)?' . $fields_meta[$i]->name . '~i', $select_stt[1], $parts)) || (isset($analyzed_sql[0]['select_expr'][$i]['expr'])
                   && isset($analyzed_sql[0]['select_expr'][$i]['column']) && $analyzed_sql[0]['select_expr'][$i]['expr'] != $analyzed_sql[0]['select_expr'][$i]['column']
            ...
            if (mb_strpos(' ' . $fields_meta[$i]->name, '`') > 0) {
            ...
            if (!$is_in_sort) {
                ...
                $cfg['Order'] = mb_strtoupper($GLOBALS['cfg']['Order']);
                ...
                if (preg_match('@[[:space:]]ASC$@i', $sort_expression)) {
                    $sort_order .= ' DESC';
                    ...
                    if (preg_match('@[[:space:]]DESC$@i', $sort_expression)) {
                        $sort_order .= ' ASC';
            ...
            $order_link_pre = '<a href="sql.php?' . $url_query . '" ' . (('horizontalflipped' == $disp_direction && 'css' == $GLOBALS['cfg']['HeaderFlipType']) ? 'style="direction: ltr; writing-mode: tb-rl;"' : '') . ' title="' . $GLOBALS['strSort'] . '"' . $order_onmouse . '>';

            $order_link_post = '</a>';

            $order_link_content = ('horizontalflipped' == $disp_direction && 'fake' == $GLOBALS['cfg']['HeaderFlipType'] ? PMA_flipstring(htmlspecialchars($fields_meta[$i]->name, ENT_QUOTES | ENT_HTML5), "<br>\n") : htmlspecialchars($fields_meta[$i]->name, ENT_QUOTES | ENT_HTML5));
            ...
            if ('horizontal' == $disp_direction || 'horizontalflipped' == $disp_direction) {
                echo "\n"; ?>
<th <?php echo $column_style; ?> <?php if ('horizontalflipped' == $disp_direction) {
                    echo 'valign="bottom"';
                } ?>>
...
<th <?php echo $column_style; ?> <?php if ('horizontalflipped' == $disp_direction) {
                    echo 'valign="bottom"';
                } ?>  <?php echo('horizontalflipped' == $disp_direction && 'css' == $GLOBALS['cfg']['HeaderFlipType'] ? 'style="direction: ltr; writing-mode: tb-rl;"' : ''); ?>>
    <?php echo $comments_table_wrap_pre; ?>
    <?php echo ('horizontalflipped' == $disp_direction && 'fake' == $GLOBALS['cfg']['HeaderFlipType'] ? PMA_flipstring(htmlspecialchars($fields_meta[$i]->name, ENT_QUOTES | ENT_HTML5), "<br>\n") : htmlspecialchars($fields_meta[$i]->name, ENT_QUOTES | ENT_HTML5)) . "\n"; ?>
    <?php echo $comments_table_wrap_post; ?>
</th>
                <?php
            }
    ...
    if ($GLOBALS['cfg']['ModifyDeleteAtRight']
        && ('nn' != $is_display['edit_lnk'] || 'nn' != $is_display['del_lnk'])
        && '1' == $is_display['text_btn']) {
        $vertical_display['emptyafter'] = ('nn' != $is_display['edit_lnk'] && 'nn' != $is_display['del_lnk']) ? 3 : 1;
 ...
 * @global  int  the total number of fields returned by the sql query
 * @global  array    informations used with vertical display mode
 * @global  string   the display mode (horizontal/vertical/horizontalflipped)
 * @global  int  the number of row to display between two table headers
 * @global  bool  whether to limit the number of displayed characters of
...
<!-- Results table body -->
    ...
    while (false !== ($row = PMA_DBI_fetch_row($dt_result))) {
        // lem9: "vertical display" mode stuff

        if ((0 != $row_no) && (0 != $repeat_cells) && !($row_no % $repeat_cells) && ('horizontal' == $disp_direction || 'horizontalflipped' == $disp_direction)) {
            echo '<tr>' . "\n";
...
<tr<?php echo $on_mouse; ?>>
        ...
        if ('nn' != $is_display['edit_lnk'] || 'nn' != $is_display['del_lnk']) {
            ...
            if ('ur' == $is_display['edit_lnk'] /* || $is_display['edit_lnk'] == 'dr' */) {
            ...
            // 1.2.1 Modify link(s)
            if ('ur' == $is_display['edit_lnk']) { // update row case
//                    $lnk_goto = 'sql.php'
...
// to reduce the length of the URL, because of some browsers limitations:
            ...
            if ('dr' == $is_display['del_lnk']) { // delete row case
                $lnk_goto = 'sql.php'
                          . '?' . str_replace('&amp;', '&', $url_query)
                          . '&sql_query=' . urlencode($url_sql_query)
                          . '&zero_rows=' . urlencode(htmlspecialchars($GLOBALS['strDeleted'], ENT_QUOTES | ENT_HTML5))
                          . '&goto=' . (empty($goto) ? 'tbl_properties.php' : $goto);
                ...
                if ('kp' == $is_display['del_lnk']) { // kill process case
                    $lnk_goto = 'sql.php'
                          . '?' . str_replace('&amp;', '&', $url_query)
                          . '&sql_query=' . urlencode($url_sql_query)
                          . '&goto=main.php';
            ...
            if ($GLOBALS['cfg']['ModifyDeleteAtLeft']
                && ('horizontal' == $disp_direction || 'horizontalflipped' == $disp_direction)) {
                $doWriteModifyAt = 'left';

                require __DIR__ . '/libraries/display_tbl_links.lib.php';
            } // end if (1.3)

            echo(('horizontal' == $disp_direction || 'horizontalflipped' == $disp_direction) ? "\n" : '');
        } // end if (1)
        ...
        for ($i = 0; $i < $fields_cnt; ++$i) {
            ...
            if ('vertical' == $disp_direction && (!isset($GLOBALS['printview']) || ('1' != $GLOBALS['printview']))) {
                if (true === $GLOBALS['cfg']['BrowsePointerColor']) {
            ...
            if ($GLOBALS['cfgRelation']['mimework'] && $GLOBALS['cfg']['BrowseMIME']) {
                if (isset($GLOBALS['mime_map'][$meta->name]['mimetype']) && isset($GLOBALS['mime_map'][$meta->name]['transformation']) && !empty($GLOBALS['mime_map'][$meta->name]['transformation'])) {
                    ...
                    if (file_exists('./libraries/transformations/' . $include_file)) {
                        ...
                        require_once __DIR__ . '/libraries/transformations/' . $include_file;

                        if (function_exists('PMA_transformation_' . $transformfunction_name)) {
                            $transform_function = 'PMA_transformation_' . $transformfunction_name;

                            $transform_options = PMA_transformation_getOptions(($GLOBALS['mime_map'][$meta->name]['transformation_options'] ?? ''));
            ...
            if (1 == $meta->numeric) {
                ...
                //       $GLOBALS['xoopsDB']->fetchBoth() is MYSQL_BOTH, so we always get
                        ...
                        if (isset($map[$meta->name])) {
                            ...
                            if (!empty($map[$meta->name][2])) {
                                ...
                                if ($dispresult && PMA_DBI_num_rows($dispresult) > 0) {
                                    [$dispval] = PMA_DBI_fetch_row($dispresult, 0);
                                } else {
                            ...
                            if (isset($GLOBALS['printview']) && '1' == $GLOBALS['printview']) {
                                $vertical_display['data'][$row_no][$i] .= ($transform_function != $default_function ? $transform_function($row[$i], $transform_options, $meta) : $transform_function($row[$i], [], $meta)) . ' <code>[-&gt;' . $dispval . ']</code>';
                            } else {
                                $title = (!empty($dispval)) ? ' title="' . htmlspecialchars($dispval, ENT_QUOTES | ENT_HTML5) . '"' : '';
                                ...
                                $vertical_display['data'][$row_no][$i] .= '<a href="sql.php?'
                                                                   ...
                                                                   . ($transform_function != $default_function ? $transform_function($row[$i], $transform_options, $meta) : $transform_function($row[$i], [], $meta)) . '</a>';
                            }
                        } else {
                            $vertical_display['data'][$row_no][$i] .= ($transform_function != $default_function ? $transform_function($row[$i], $transform_options, $meta) : $transform_function($row[$i], [], $meta));
                        }
                ...
                if (false === $GLOBALS['cfg']['ShowBlob'] && mb_stristr($meta->type, 'BLOB')) {
                    ...
                    if (mb_stristr($field_flags, 'BINARY')) {
                        $blobtext = '[BLOB';

                        if (!isset($row[$i]) || null === $row[$i]) {
                            $blobtext .= ' - NULL';
                        } elseif (isset($row[$i])) {
                            $blob_size = PMA_formatByteDown(mb_strlen($row[$i]), 3, 1);
                        ...
                        $blobtext = ($default_function != $transform_function ? $transform_function($blobtext, $transform_options, $meta) : $default_function($blobtext, [], $meta));
                        ...
                        if (!isset($row[$i]) || null === $row[$i]) {
                            ...
                            if ('' != $row[$i]) {
                                ...
                                if (PMA_strlen($row[$i]) > $GLOBALS['cfg']['LimitChars'] && (1 != $dontlimitchars)) {
                        ...
                        if ('' != $row[$i]) {
                            ...
                            if (isset($map[$meta->name])) {
                                ...
                                if (!empty($map[$meta->name][2])) {
                                    ...
                                    if ($dispresult && PMA_DBI_num_rows($dispresult) > 0) {
                                        [$dispval] = PMA_DBI_fetch_row($dispresult);
                                    } else {
                                        $dispval = $GLOBALS['strLinkNotFound'];
                                    }
...
function PMA_displayVerticalTable()
{
    ...
    if ($GLOBALS['cfg']['ModifyDeleteAtLeft'] && is_array($vertical_display['row_delete']) && (count($vertical_display['row_delete']) > 0 || !empty($vertical_display['textbtn']))) {
        ...
        foreach ($vertical_display['row_delete'] as $key => $val) {
            if ((0 != $foo_counter) && (0 != $repeat_cells) && !($foo_counter % $repeat_cells)) {
                echo '<th class="td">&nbsp;</th>' . "\n";
            }
    ...
    foreach ($vertical_display['desc'] as $key => $val) {
        ...
        foreach ($vertical_display['rowdata'][$key] as $subkey => $subval) {
            if ((0 != $foo_counter) && (0 != $repeat_cells) and !($foo_counter % $repeat_cells)) {
                echo $val;
            }
 ...
 * @global  int  the total number of rows returned by the sql query
 *                   without any programmatically appended "LIMIT" clause
 * @global  int  the current postion of the first record to be
 *                   displayed
 * @global  array    the list of fields properties
 * @global  int  the total number of fields returned by the sql query
 * @global  array    informations used with vertical display mode
...
function PMA_displayTable($dt_result, &$the_disp_mode, $analyzed_sql)
{
    ...
    if ('1' == $is_display['nav_bar']) {
        if (!isset($pos)) {
            $pos = 0;
        }

        if ('all' == $GLOBALS['session_max_rows']) {
            $pos_next = 0;
    ...
    if ('1' == $is_display['nav_bar'] && isset($pos_next)) {
        ...
        $last_shown_rec = ('all' == $GLOBALS['session_max_rows'] || $pos_next > $total) ? $total - 1 : $pos_next - 1;
    ...
    if (!isset($table) || 0 == mb_strlen(trim($table))) {
    ...
    if ('dr' == $is_display['del_lnk'] && 'kp' != $is_display['del_lnk']) {
        $delete_text = 'dr' == $is_display['del_lnk'] ? $GLOBALS['strDelete'] : $GLOBALS['strKill'];
        ...
        if ($cfg['PropertiesIconic']) {
            ...
            if (PMA_USR_BROWSER_AGENT != 'IE') {
                echo '                    <button class="mult_submit" type="submit" name="submit_mult" value="row_edit" title="' . $GLOBALS['strEdit'] . '">' . "\n"
                   . '<img src="' . $GLOBALS['pmaThemeImage'] . 'b_edit.png" title="' . $GLOBALS['strEdit'] . '" alt="' . $GLOBALS['strEdit'] . '" width="16" height="16" align="absmiddle">' . (('both' == $propicon) ? '&nbsp;' . $GLOBALS['strEdit'] : '') . "\n"
                   . '</button>';

                echo '&nbsp;<button class="mult_submit" type="submit" name="submit_mult" value="row_delete" title="' . $delete_text . '">' . "\n"
                   . '<img src="' . $GLOBALS['pmaThemeImage'] . 'b_drop.png" title="' . $delete_text . '" alt="' . $delete_text . '" width="16" height="16" align="absmiddle">' . (('both' == $propicon) ? '&nbsp;' . $delete_text : '') . "\n"
                   . '</button>';

                if ('SELECT' == $analyzed_sql[0]['querytype']) {
                    echo '&nbsp;<button class="mult_submit" type="submit" name="submit_mult" value="row_export" title="' . $GLOBALS['strExport'] . '">' . "\n"
                       . '<img src="' . $GLOBALS['pmaThemeImage'] . 'b_tblexport.png" title="' . $GLOBALS['strExport'] . '" alt="' . $GLOBALS['strExport'] . '" width="16" height="16" align="absmiddle">' . (('both' == $propicon) ? '&nbsp;' . $GLOBALS['strExport'] : '') . "\n"
                       . '</button>';
                }
            } else {
                echo '                    <input type="image" name="submit_mult_edit" value="row_edit" title="' . $GLOBALS['strEdit'] . '" src="' . $GLOBALS['pmaThemeImage'] . 'b_edit.png">' . (('both' == $propicon) ? '&nbsp;' . $GLOBALS['strEdit'] : '');

                echo '&nbsp;<input type="image" name="submit_mult" value="row_delete" title="' . $delete_text . '" src="' . $GLOBALS['pmaThemeImage'] . 'b_drop.png">' . (('both' == $propicon) ? '&nbsp;' . $delete_text : '');

                if ('SELECT' == $analyzed_sql[0]['querytype']) {
                    echo '&nbsp;<input type="image" name="submit_mult_export" value="row_export" title="' . $GLOBALS['strExport'] . '" src="' . $GLOBALS['pmaThemeImage'] . 'b_tblexport.png">' . (('both' == $propicon) ? '&nbsp;' . $GLOBALS['strExport'] : '');
                }
            }

            echo "\n";
        } else {
            echo '                    <input type="submit" name="submit_mult" value="' . htmlspecialchars($GLOBALS['strEdit'], ENT_QUOTES | ENT_HTML5) . '" title="' . $GLOBALS['strEdit'] . '">' . "\n";

            echo '&nbsp;<input type="submit" name="submit_mult" value="' . htmlspecialchars($delete_text, ENT_QUOTES | ENT_HTML5) . '" title="' . $delete_text . '">' . "\n";

            if ('SELECT' == $analyzed_sql[0]['querytype']) {
                echo '&nbsp;<input type="submit" name="submit_mult" value="' . htmlspecialchars($GLOBALS['strExport'], ENT_QUOTES | ENT_HTML5) . '" title="' . $GLOBALS['strExport'] . '">' . "\n";
            }
        }

        echo '<input type="hidden" name="sql_query" value="' . htmlspecialchars($sql_query, ENT_QUOTES | ENT_HTML5) . '">' . "\n";
...
{
    $buffer = htmlspecialchars($buffer, ENT_QUOTES | ENT_HTML5);
