 ...
 *
 * @param   the      $string string to convert
 *
...
function PMA_exportData($db, $table, $crlf, $error_url, $sql_query)
{
    ...
    while (false !== ($record = PMA_DBI_fetch_assoc($result))) {
        $buffer = '';

        // print each row

        for ($i = 0; $i < $columns_cnt; $i++) {
            if (isset($record[$columns[$i]]) && (!function_exists('is_null') || null !== $record[$columns[$i]])) {
...
function PMA_exportStructure($db, $table, $crlf, $error_url, $do_relation = false, $do_comments = false, $do_mime = false, $dates = false)
{
    ...
    while (false !== ($key = PMA_DBI_fetch_assoc($keys_result))) {
        if (0 == $key['Non_unique']) {
    ...
    while (false !== ($row = PMA_DBI_fetch_assoc($result))) {
        ...
        $local_buffer = $field_name . "\000" . $type . "\000" . (('' == $row['Null']) ? $GLOBALS['strNo'] : $GLOBALS['strYes']) . "\000" . ($row['Default'] ?? '');
        ...
        if (in_array($field_name, $unique_keys, true)) {
            $pos = mb_strpos($local_buffer, "\000");
