...
if ($foreigners && isset($foreigners[$field])) {
    ...
    if ((isset($override_total) && true === $override_total) || $the_total < 200) {
        // foreign_display can be FALSE if no display field defined:

        $foreign_display = PMA_getDisplayField($foreign_db, $foreign_table);

        $dispsql = 'SELECT ' . PMA_backquote($foreign_field)
                         . ((false === $foreign_display) ? '' : ', ' . PMA_backquote($foreign_display))
                         . ' FROM ' . PMA_backquote($foreign_db) . '.' . PMA_backquote($foreign_table)
                         . ((false === $foreign_display) ? '' : ' ORDER BY ' . PMA_backquote($foreign_table) . '.' . PMA_backquote($foreign_display))
                         . ($foreign_limit ?? '');
        ...
        if ($disp) {
            ...
            while (false !== ($single_disp_row = @PMA_DBI_fetch_assoc($disp))) {
