...
if (isset($cfg['Servers'])) {
    for ($i = 1; (!empty($cfg['Servers'][$i]['host']) || (isset($cfg['Servers'][$i]['connect_type']) && 'socket' == $cfg['Servers'][$i]['connect_type'])); $i++) {
        ...
        if (!isset($cfg['Servers'][$i]['auth_type'])) {
            $cfg['Servers'][$i]['auth_type'] = (isset($cfg['Servers'][$i]['adv_auth']) && $cfg['Servers'][$i]['adv_auth']) ? 'http' : 'config';
...
if (!isset($cfg['BrowseMarkerColor'])) {
    if (isset($cfgBrowseMarkerColor)) {
        ...
        if (isset($cfg['BrowseMarkRow'])) {
            $cfg['BrowseMarkerColor'] = (!empty($cfg['BrowsePointerColor']) && !empty($cfg['BrowseMarkRow'])) ? '#FFCC99' : '';

            unset($cfg['BrowseMarkRow']);
        } else {
            if (isset($cfgBrowseMarkRow)) {
                $cfg['BrowseMarkerColor'] = (!empty($cfg['BrowsePointerColor']) && !empty($cfgBrowseMarkRow)) ? '#FFCC99' : '';
