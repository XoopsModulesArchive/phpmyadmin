...
require_once $cfgfile_to_load;
// Include failed
if (!isset($cfgServers) && !isset($cfg['Servers'])) {
    // Creates fake settings

    $cfg = [
'DefaultLang' => 'en-iso-8859-1',
'AllowAnywhereRecoding' => false,
];

    // Loads the language file

    require_once __DIR__ . '/libraries/select_lang.lib.php';
...
if (isset($cfg['FileRevision'])) {
    ...
    $cfg['FileRevision'] = [1, 1];
}
if ($cfg['FileRevision'][0] < 2 || (2 == $cfg['FileRevision'][0] && $cfg['FileRevision'][1] < 36)) {
    require_once __DIR__ . '/libraries/config_import.lib.php';
}
 ...
 */
require_once __DIR__ . '/libraries/defines.lib.php';
...
if ('auto' == mb_strtolower($cfg['OBGzip'])) {
...
if (!isset($_COOKIE['pma_theme']) || empty($_COOKIE['pma_theme'])) {
    ...
    if ('' != $cfg['ThemePath'] && false !== $cfg['ThemePath']) {
        $tmp_theme_mainpath = './' . $cfg['ThemePath'];

        $tmp_theme_fullpath = './' . $cfg['ThemePath'] . '/' . $cfg['ThemeDefault'];

        if (@is_dir($tmp_theme_mainpath)) {
            if (isset($cfg['ThemeDefault']) && 'original' != $cfg['ThemeDefault'] && @is_dir($tmp_theme_fullpath)) {
                $ThemeDefaultOk = true;
            }
        }
    }

    if (true === $ThemeDefaultOk) {
...
if (false === $is_minimum_common) {
    ...
    require_once __DIR__ . '/libraries/url_generating.lib.php';
     ...
     * @param mixed $unparsed_sql
     *
     * @return  string   the formatted sql
     *
     * @global  array    the configuration array
     * @global  bool  whether the current statement is a multiple one or not
     *
    ...
    function PMA_formatSql($parsed_sql, $unparsed_sql = '')
    {
        ...
        if (!is_array($parsed_sql)) {
            ...
            $formatted_sql = '<pre>' . "\n"
                            . (('none' == $cfg['SQP']['fmtType'] && '' != $unparsed_sql) ? $unparsed_sql : $parsed_sql) . "\n"
                            . '</pre>';
    ...
    function PMA_mysqlDie(
        ...
        if (!function_exists('PMA_SQP_isError') || PMA_SQP_isError()) {
            $formatted_sql = htmlspecialchars($the_query, ENT_QUOTES | ENT_HTML5);
        } else {
        ...
        if (!empty($the_query) && !mb_strstr($the_query, 'connect')) {
            ...
            if (mb_strstr(mb_strtolower($formatted_sql), 'select')) { // please show me help to the error on select
        ...
        if (!empty($back_url) && $exit) {
            $goto_back_url = '<a href="' . (mb_strstr($back_url, '?') ? $back_url . '&amp;no_history=true' : $back_url . '?no_history=true') . '">&nbsp;';
     ...
     * @param mixed $charset_introducer
 ...
 *
 * @param mixed $only_db_check
 * @param mixed $dbh
 * @param mixed $dblist_cnt
 * @param mixed $rs
    ...
    if (!$dblist_cnt
        ...
        if ('Y' != $row['Select_priv']) {
            ...
            if ($rs && @PMA_DBI_num_rows($rs)) {
                ...
                while (false !== ($row = PMA_DBI_fetch_assoc($rs))) {
                ...
                if (isset($uva_mydbs['%'])) {
                    while (false !== ($uva_row = PMA_DBI_fetch_row($uva_alldbs))) {
                        $dblist[] = $uva_row[0];
                    } // end while
                } // end if
                else {
...
if (false === $is_minimum_common) {
    ...
    if (empty($cfg['PmaAbsoluteUri'])) {
        ...
        if (!empty($HTTP_SCHEME)) {
            ...
            $cfg['PmaAbsoluteUri'] = ((!empty($HTTPS) && 'off' != mb_strtolower($HTTPS)) ? 'https' : 'http') . '://';
        }

        $port_in_HTTP_HOST = (mb_strpos($HTTP_HOST, ':') > 0);
        ...
        if (!(!$port_in_HTTP_HOST && !empty($SERVER_PORT) && (80 == $SERVER_PORT || 443 == $SERVER_PORT))) {
        ...
        if (isset($REQUEST_URI)) {
            $cfg['PmaAbsoluteUri'] .= mb_substr($REQUEST_URI, 0, mb_strrpos($REQUEST_URI, '/') + 1);
        } else {
            if (isset($PATH_INFO)) {
                $cfg['PmaAbsoluteUri'] .= mb_substr($PATH_INFO, 0, mb_strrpos($PATH_INFO, '/') + 1);
            } else {
                $cfg['PmaAbsoluteUri'] .= mb_substr($PHP_SELF, 0, mb_strrpos($PHP_SELF, '/') + 1);
            }
        }
        ...
        if (false === $cfg['PmaAbsoluteUri_DisableWarning']) {
        ...
        if ('/' != mb_substr($cfg['PmaAbsoluteUri'], -1)) {
            $cfg['PmaAbsoluteUri'] .= '/';
        }
        ...
        if ('http://' != mb_substr($cfg['PmaAbsoluteUri'], 0, 7) && 'https://' != mb_substr($cfg['PmaAbsoluteUri'], 0, 8)) {
            if (!empty($_SERVER)) {
                $SERVER_ARRAY = '_SERVER';
            } else {
                $SERVER_ARRAY = 'GLOBALS';
            ...
            $cfg['PmaAbsoluteUri'] = ((!empty($HTTPS) && 'off' != mb_strtolower($HTTPS)) ? 'https' : 'http') . ':'
                                            . ('//' == mb_substr($cfg['PmaAbsoluteUri'], 0, 2) ? '' : '//')
                                            . $cfg['PmaAbsoluteUri'];
        }
    }
    ...
    foreach ($cfg['Servers'] as $key => $val) {
        ...
        if (('tcp' == $val['connect_type']) && empty($val['host'])) {
        ...
        if (('socket' == $val['connect_type']) && empty($val['host']) && empty($val['verbose'])) {
     ...
     */
        ...
        if (isset($cfg['Servers'][$server])) {
            ...
            require_once __DIR__ . '/libraries/database_interface.lib.php';
            ...
            require_once __DIR__ . '/libraries/auth/' . $cfg['Server']['auth_type'] . '.auth.lib.php';
            ...
            if (isset($cfg['Server']['AllowDeny']) && isset($cfg['Server']['AllowDeny']['order'])) {
                require_once __DIR__ . '/libraries/ip_allow_deny.lib.php';

                $allowDeny_forbidden = false; // default

                if ('allow,deny' == $cfg['Server']['AllowDeny']['order']) {
                    ...
                    if ('deny,allow' == $cfg['Server']['AllowDeny']['order']) {
                        if (PMA_allowDeny('deny')) {
                            $allowDeny_forbidden = true;
                        }
                        ...
                        if ('explicit' == $cfg['Server']['AllowDeny']['order']) {
                            if (PMA_allowDeny('allow')
                    && !PMA_allowDeny('deny')) {
                                $allowDeny_forbidden = false;
                            } else {
        ...
        // The user can work with only some databases
            ...
            require_once __DIR__ . '/libraries/sqlvalidator.lib.php';
            ...
            if ($dblist_cnt) {
                ...
                for ($i = 0; $i < $dblist_cnt; $i++) {
                    ...
                    if ('*' == $dblist[$i] && false === $dblist_asterisk_bool) {
                        ...
                        foreach ($dblist_full as $dbl_key => $dbl_val) {
                            if (!in_array($dbl_val, $dblist, true)) {
                                $true_dblist[] = $dbl_val;
                            }
                        }
                    ...
                    if ($is_show_dbs && preg_match('(^|[^\])(_|%)', $dblist[$i])) {
                        ...
                        if (0 == $i
                        && (1045 == mb_substr(PMA_DBI_getError($dbh), 1, 4))) {
                        ...
                        while (false !== ($row = @PMA_DBI_fetch_row($rs))) {
                            $true_dblist[] = $row[0];
                        } // end while
    ...
    function PMA_jsFormat($a_string = '', $add_backquotes = true)
    {
        if (is_string($a_string)) {
            $a_string = htmlspecialchars($a_string, ENT_QUOTES | ENT_HTML5);
    ...
    function PMA_countRecords($db, $table, $ret = false)
    {
        ...
        if (!isset($num)) {
            ...
            [$num] = ($result) ? PMA_DBI_fetch_row($result) : [0];
    ...
    function PMA_showMessage($message)
    {
        ...
        if (isset($GLOBALS['reload']) && $GLOBALS['reload']) {
            ...
            $reload_url = './left.php?' . PMA_generate_common_url(($GLOBALS['db'] ?? ''), '', '&')
            ?>
...
<div align="<?php echo $GLOBALS['cell_align_left']; ?>">
    ...
    <tr>
        <th<?php echo ('original' != $GLOBALS['theme']) ? ' class="tblHeaders"' : ' bgcolor="' . $cfg['ThBgcolor'] . '"'; ?>>
            <b><?php echo $message; ?></b>
        </th>
    </tr>
        <?php
        if (true === $cfg['ShowSQL'] && (!empty($GLOBALS['sql_query']) || !empty($GLOBALS['display_query']))) {
            $local_query = !empty($GLOBALS['display_query']) ? $GLOBALS['display_query'] : (('none' == $cfg['SQP']['fmtType'] && isset($GLOBALS['unparsed_sql']) && '' != $GLOBALS['unparsed_sql']) ? $GLOBALS['unparsed_sql'] : $GLOBALS['sql_query']);
    ...
    <tr>
        <td bgcolor="<?php echo $cfg['BgcolorOne']; ?>">
            ...
            if (isset($new_line)) {
                ...
                $query_base = PMA_sqlAddslashes(htmlspecialchars($local_query, ENT_QUOTES | ENT_HTML5));
            ...
            if (isset($cfg['SQLQuery']['Explain'])
                ...
                if (preg_match('@^SELECT[[:space:]]+@i', $local_query)) {
                    ...
                    if (preg_match('@^EXPLAIN[[:space:]]+SELECT[[:space:]]+@i', $local_query)) {
                        $explain_link .= urlencode(mb_substr($local_query, 8)) . '">' . $GLOBALS['strNoExplain'];
                    } else {
                        $explain_link = '';
                    }
                }
            ...
            if (isset($cfg['SQLQuery']['ShowAsPHP'])
                ...
                if (isset($GLOBALS['show_as_php']) && '1' == $GLOBALS['show_as_php']) {
                    $php_link .= '&nbsp;[<a href="read_dump.php'
                              . $url_qpart
                              . '&amp;show_query=1'
                              . '&amp;sql_query=' . urlencode($local_query)
            ...
            if (isset($cfg['SQLValidator']['use'])
                && true === $cfg['SQLValidator']['use'] && isset($cfg['SQLQuery']['Validate'])
                && true === $cfg['SQLQuery']['Validate']) {
    ...
    {
        $open = mb_strpos($type_def, '(');

        $close = mb_strrpos($type_def, ')');

        if (!$open || !$close) {
            return false;
        }

        $options = mb_substr($type_def, $open + 2, $close - $open - 3);
    ...
    */
    ...
    function PMA_printTab($text, $link, $args = '', $attr = '', $class = '', $sep = '?', $active = false)
    {
        ...
        if ((
            (!isset($GLOBALS['active_page']) && basename($PHP_SELF) == $link) || $active || (isset($GLOBALS['active_page']) && $GLOBALS['active_page'] == $link)
        ) && ($text != $GLOBALS['strEmpty'] && $text != $GLOBALS['strDrop'])) {
            $addclass = 'Active';
        }

        if ($text == $GLOBALS['strEmpty'] && $text == $GLOBALS['strDrop']) {
            $addclass = 'Drop';
        }
    ...
    function PMA_linkOrButton($url, $message, $js_conf, $allow_button = true)
    {
        ...
        if (mb_strlen($url) <= 1000) {
            ...
            foreach ($query_parts as $query_pair) {
                [$eachvar, $eachval] = explode('=', $query_pair);

                $link_or_button .= '            <input type="hidden" name="' . str_replace('amp;', '', $eachvar) . '" value="' . htmlspecialchars(urldecode($eachval), ENT_QUOTES | ENT_HTML5) . '">' . "\n";
            } // end while

            if (mb_stristr($message, '<img')) {
                $link_or_button .= '            <input type="image" src="' . preg_replace('@^.*src="(.*)".*$@si', '\1', $message) . '" value="'
                                   . htmlspecialchars(preg_replace('@^.*alt="(.*)".*$@si', '\1', $message), ENT_QUOTES | ENT_HTML5) . '">' . "\n" . '</form>' . "\n";
            } else {
    ...
    if (@function_exists('mb_convert_encoding')
        ...
        require_once __DIR__ . '/libraries/kanji-encoding.lib.php';
    ...
    {
        if (mb_substr($file, -1 * mb_strlen($extension)) == $extension) {
            return true;
        }

        if ($GLOBALS['cfg']['GZipDump'] && @function_exists('gzopen')) {
            if (mb_substr($file, -3 - mb_strlen($extension)) == $extension . '.gz') {
                return true;
            }
        }

        if ($GLOBALS['cfg']['BZipDump'] && @function_exists('bzdecompress')) {
            if (mb_substr($file, -4 - mb_strlen($extension)) == $extension . '.bz2') {
                return true;
            }
        }
    ...
    function PMA_getUvaCondition($handle, $fields_cnt, $fields_meta, $row)
    {
        ...
        for ($i = 0; $i < $fields_cnt; ++$i) {
            ...
            if ('real' == $meta->type) {
                ...
                if (PMA_MYSQL_INT_VERSION >= 40100 && ('string' == $meta->type || 'blob' == $meta->type)) {
