...
/* $Id: defines.lib.php,v 2.16 2004/06/28 20:45:54 lem9 Exp $ */
...
if (!defined('PMA_IS_WINDOWS')) {
    if (defined('PHP_OS') && mb_stristr(PHP_OS, 'win')) {
...
if (!defined('PMA_IS_IIS')) {
    ...
    && mb_stristr($_SERVER['SERVER_SOFTWARE'], 'Microsoft/IIS')) {
        define('PMA_IS_IIS', 1);
    } else {
        define('PMA_IS_IIS', 0);
    }
...
if (!defined('PMA_IS_GD2')) {
    if ('yes' == $cfg['GD2Available']) {
        define('PMA_IS_GD2', 1);
    } elseif ('no' == $cfg['GD2Available']) {
        define('PMA_IS_GD2', 0);
    } else {
...
if (!defined('PMA_USR_OS')) {
    ...
    if (mb_strstr($HTTP_USER_AGENT, 'Win')) {
        ...
        if (mb_strstr($HTTP_USER_AGENT, 'Mac')) {
            define('PMA_USR_OS', 'Mac');
        } else {
            if (mb_strstr($HTTP_USER_AGENT, 'Linux')) {
                define('PMA_USR_OS', 'Linux');
            } else {
                if (mb_strstr($HTTP_USER_AGENT, 'Unix')) {
                    define('PMA_USR_OS', 'Unix');
                } else {
                    if (mb_strstr($HTTP_USER_AGENT, 'OS/2')) {
    ...
    if (preg_match('@Opera(/| )([0-9].[0-9]{1,2})@', $HTTP_USER_AGENT, $log_version)) {
        ...
        if (preg_match('@MSIE ([0-9].[0-9]{1,2})@', $HTTP_USER_AGENT, $log_version)) {
            ...
            if (preg_match('@OmniWeb/([0-9].[0-9]{1,2})@', $HTTP_USER_AGENT, $log_version)) {
                define('PMA_USR_BROWSER_VER', $log_version[1]);
    ...
    // Konqueror 3.0.3 says Konqueror/3
                ...
                if (preg_match('@(Konqueror/)(.*)(;)@', $HTTP_USER_AGENT, $log_version)) {
                    define('PMA_USR_BROWSER_VER', $log_version[2]);

                    define('PMA_USR_BROWSER_AGENT', 'KONQUEROR');
                } else {
                    if (preg_match('@Mozilla/([0-9].[0-9]{1,2})@', $HTTP_USER_AGENT, $log_version)
