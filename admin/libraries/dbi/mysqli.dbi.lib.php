...
function PMA_DBI_connect($user, $password)
{
    ...
    if ('tcp' == mb_strtolower($cfg['Server']['connect_type'])) {
        $cfg['Server']['socket'] = '';
    }

    $server_socket = (empty($cfg['Server']['socket'])) ? false : $cfg['Server']['socket'];
...
function PMA_mysqli_fetch_array($result, $type = false)
{
    ...
    for ($i = 0; $i < $num; $i++) {
        if (!$meta) {
            ...
            if (mb_stristr($fields[$i]->type, 'BLOB') || mb_stristr($fields[$i]->type, 'BINARY')) {
                if (isset($data[$i])) {
                    $ret[$i] = $data[$i];
                }
...
//            return FALSE;
    ...
    } elseif ($GLOBALS['xoopsDB']->errno($link)) {
